...
- **Админ** (задаётся в `TELEGRAM_ADMIN_ID`): может удалять сообщения в общем чате (кнопка ✕ у каждого сообщения) и писать в личку любому пользователю. По умолчанию админ — пользователь с ID `388008256`. Чтобы сменить: добавьте переменную **TELEGRAM_ADMIN_ID** в Vercel (ваш Telegram User ID).
