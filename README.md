...
- **Админ** (задаётся в `TELEGRAM_ADMIN_ID`): может удалять сообщения (✕), банить пользователей, редактировать свои, использовать контекстное меню по long‑press. Под ID админов показывается «(админ)». Дефолтные админы: Анна (2144406710), Роман (5053253480), вы. В Vercel задайте **TELEGRAM_ADMIN_ID** — Telegram User ID каждого через запятую (узнать в @userinfobot). Без этого метка «(админ)» и кнопки админа не появятся.
...
1. Зайди в **Vercel** → проект → **Settings** → **Environment Variables**.
2. Найди переменную **CRON_SECRET** или **PIKHANINA_RESET_SECRET** и скопируй её значение (без пробелов в начале и конце).
...
Если ошибка 401/403 — секрет не совпадает, перепроверь значение в Vercel и что переменная сохранена (после смены переменных сделай Redeploy).
