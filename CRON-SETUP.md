...
После публикации новой новости вызовите:

```bash
curl -X POST "https://poker-app-ebon.vercel.app/api/gazette-notify" \
  -H "Content-Type: application/json" \
  -H "X-Cron-Secret: ВАШ_CRON_SECRET" \
  -d '{"headline": "КОНЕЦ РОЗЫГРЫША! Клуб в трауре...", "postToChat": true}'
```

- **message** — текст пуша в Telegram подписчикам (если не указан — стандартный).
- **headline** (или **title**) — заголовок новости; уйдёт в пост в чат и в пуш.
- **postToChat: true** — дополнительно отправить сообщение со ссылкой в общий чат клуба.

В Vercel добавьте **MINI_APP_URL** (ссылка на Mini App, например `https://t.me/YourBot/app`) — она будет вставлена в пост в чат.
