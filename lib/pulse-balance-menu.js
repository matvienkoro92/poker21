...
function requisiteButtonText(count, balance) {
  const amount = balanceButtonText({ cents: balance?.paymentCents ?? 0, usdCents: balance?.paymentUsdCents ?? 0 }).replace('💰 Клубный баланс: ', '');
...
async function refreshMenu(chatId, telegram) {
  ...
  try {
    ...
    for (const button of menu.markup.inline_keyboard.flat()) if (button.callback_data === 'paymenu:list') {
      const count = button.text.match(/—\s*(\d+)/)?.[1] ?? null;
...
module.exports = { balanceButtonText, requisiteButtonText, trackMenu, refreshMenu };
