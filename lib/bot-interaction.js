...
async function run(req, res, handler, notify) {
  ...
  return context.run(state, async () => {
    ...
    try {
      ...
      // Keep the current screen intact. The technical details remain in logs,
      // while Telegram only receives an invisible acknowledgement to stop its spinner.
      if (cb?.id) await notify('answerCallbackQuery', { callback_query_id: cb.id }).catch(() => {});
      return res.status(200).json({ ok: false, errorId: state.id });
    }
