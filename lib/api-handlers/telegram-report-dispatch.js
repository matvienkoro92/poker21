...
const { refreshMenu } = require("../pulse-balance-menu");
...
async function updateReportBalances(mode) {
  ...
  for (let index = 0; index < targets.length; index += 1) {
    ...
    await refreshMenu(target.chatId, telegram);
...
async function sendWeeklyRecommendations(mode) {
  const targets = await balanceTargets();
  const periods = require("../../data/union-periods.json").periods || [];
  const results = [];
  for (const target of targets) {
    const prior = periods.filter(p => p.endDate < target.report.startDate).sort((a,b) => b.endDate.localeCompare(a.endDate))[0];
    const previous = (target.type === "union" ? prior?.leagueReports?.reports : prior?.clubReports?.reports)?.find(r =>
      String(target.type === "union" ? r.leagueId : r.clubId) === String(target.type === "union" ? target.report.leagueId : target.report.clubId));
    const currentFee = Number(target.report.metrics.commission);
    const priorFee = Number(previous?.metrics?.commission);
    const money = n => n.toLocaleString("ru-RU", {minimumFractionDigits:2,maximumFractionDigits:2});
    const lines = [`Рекомендации по отчёту «${target.name}»`, `${displayDate(target.report.startDate)}–${displayDate(target.report.endDate)}`, "", `Комиссия за неделю: ${money(currentFee)} ₽.`];
    if (previous) {
      const delta = currentFee - priorFee;
      lines.push(`Предыдущая неделя: ${money(priorFee)} ₽. Изменение: ${delta > 0 ? "+" : ""}${money(delta)} ₽.`);
      lines.push(delta < 0 ? "Проверьте, связано ли снижение с меньшим числом активных игроков или меньшей активностью постоянных игроков. Сравните эти показатели за обе недели перед изменением расписания." : delta > 0 ? "Проверьте, насколько рост распределён между игроками: сравните число активных игроков и долю крупнейших участников за обе недели, прежде чем расширять расписание." : "Сравните число активных игроков и распределение комиссии между ними: одинаковый итог недели может скрывать изменения состава участников.");
    } else lines.push("Сопоставьте комиссию с числом активных игроков; для оценки динамики потребуется отчёт за предыдущую неделю.");
    const base = {entity:target.name, chatId:target.chatId, startDate:target.report.startDate, endDate:target.report.endDate};
    if (mode === "preview") { results.push({...base,ok:true,text:lines.join("\n")}); continue; }
    const key = `poker21:telegram-report:recommendation-dispatch:${target.chatId}:${base.startDate}:${base.endDate}`;
    const claim = await redisPipeline([["SET",key,"1","NX"]],{context:"report-recommendation.claim",timeoutMs:3000});
    if (claim?.[0]?.result !== "OK") { results.push({...base,ok:true,skipped:true}); continue; }
    const sent = await telegram("sendMessage",{chat_id:target.chatId,text:lines.join("\n")});
    if (!sent.ok) await redisPipeline([["DEL",key]],{context:"report-recommendation.release",timeoutMs:3000});
    results.push({...base,ok:Boolean(sent.ok),error:sent.ok ? "" : sent.description});
  }
...
module.exports = async function handler(req, res) {
  ...
  if (["preview", "send"].includes(body.recommendationMode)) return res.status(200).json(await sendWeeklyRecommendations(body.recommendationMode));
