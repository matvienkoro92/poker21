...
async function sendWeeklyRecommendations(mode) {
  ...
  for (const target of targets) {
    const binding = target.type === "union" ? {type:"union", league:target.name, leagueId:target.report.leagueId} : {type:"club", club:target.name, clubId:target.report.clubId};
    const payload = await require("./telegram-report-webhook").weeklySummary(target.chatId, binding);
    const base = {entity:target.name, chatId:target.chatId, startDate:target.report.startDate, endDate:target.report.endDate};
    if (mode === "preview") { results.push({...base,ok:true,payload}); continue; }
    const key = `poker21:telegram-report:pulse-recommendation-dispatch:${target.chatId}:${base.startDate}:${base.endDate}`;
