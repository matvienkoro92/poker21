...
module.exports = async function handler(req, res) {
  ...
  const r2 = results[3] && results[3].result !== undefined ? results[3].result : 0;
  const r3 = results[4] && results[4].result !== undefined ? results[4].result : [];
  const unique = parseInt(r2, 10) || 0;
  const returning = countReturning(r3);
  const total = totalVisits(r3);

  let dtId = results[5] && results[5].result ? String(results[5].result).trim() : null;
