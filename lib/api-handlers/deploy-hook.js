...
module.exports = async function handler(req, res) {
  ...
  let boundAutoReportResult = { ok: false, skipped: true };
  try {
    const autoRes = await fetch(base + "/api/telegram-bound-auto-report", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-Cron-Secret": CRON_SECRET },
    });
    boundAutoReportResult = await autoRes.json().catch(() => ({ ok: false, error: "Invalid response" }));
  } catch (e) {
    boundAutoReportResult = { ok: false, error: (e.message || "unknown") };
  }
  ...
  return res.status(200).json({
    ...
    boundAutoReport: boundAutoReportResult,
  });
};
