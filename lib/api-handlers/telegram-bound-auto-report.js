...
module.exports = async function handler(req, res) {
  ...
  for (let index = 0; index < keys.length; index += 1) {
    ...
    if (Number.isFinite(balanceCents)) {
      ...
      const entry = JSON.stringify({ rub: { action: "adjust", cents: balanceDeltaCents }, usd: null, cents: balanceCents, usdCents: null, actor, timestamp });
      await redisPipeline([
      ...
      const balanceSent = await telegram("sendMessage", {
        chat_id: chatId,
        text: [
          `Автоотчёт учтён в балансе: ${balanceDeltaCents >= 0 ? "+" : ""}${format(balanceDeltaCents / 100)} ₽`,
          "",
          `<b>Текущий баланс: ${format(balanceCents / 100)} ₽</b>`,
        ].join("\n"),
        parse_mode: "HTML",
      });
