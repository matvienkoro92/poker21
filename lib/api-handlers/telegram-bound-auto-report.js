...
module.exports = async function handler(req, res) {
  ...
  const keysResult = await redisPipeline([["KEYS", `${BINDING_PREFIX}*`]], { context: "telegram-bound-auto-report.keys", timeoutMs: 4000 });
  ...
  for (let index = 0; index < keys.length; index += 1) {
    ...
    const claimed = await redisPipeline([["SET", lockKey, "1", "NX", "EX", "7776000"]], { context: "telegram-bound-auto-report.claim", timeoutMs: 3000 });
    ...
    if (!sent.ok) await redisPipeline([["DEL", lockKey]], { context: "telegram-bound-auto-report.release", timeoutMs: 3000 });
    results.push({ chatId, ok: Boolean(sent.ok), error: sent.ok ? "" : (sent.description || "Telegram error") });
  }
  return res.status(200).json({ ok: results.every((row) => row.ok), checked: keys.length, results });
};
