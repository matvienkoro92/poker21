 ...
 * GET /api/visitors-list?initData=... — для админа (JSON с uniqueThisMonth).
...
module.exports = async function handler(req, res) {
  ...
  const asAdmin = !isCron && adminUser && isAdmin(adminId);
  ...
  const monthIdsRaw = results[4]?.result;
  const monthIds = Array.isArray(monthIdsRaw) ? monthIdsRaw : [];
  ...
  if (asAdmin) {
    const gazetteSubscribers = parseInt(results[5]?.result, 10) || 0;
    const ratingSubscribers = parseInt(results[6]?.result, 10) || 0;
    const raffleSubscribers = parseInt(results[7]?.result, 10) || 0;
