...
async function sendLiveTables(chatId, messageId, binding, mode) {
  if (!hasPokerPlusConfig()) {
    const sent = await telegram("sendMessage", {
      chat_id: chatId,
      text: "Онлайн Poker21 временно недоступен: API не настроен.",
...
module.exports = async function handler(req, res) {
  ...
  if (clubBinding && (!isMainReportChat(chatId) || mainBalanceOnlyCommand)) {
    ...
    if (liveTablesCommand) {
      const sent = await sendLiveTables(chatId, message.message_id, clubBinding, liveTablesCommand);
