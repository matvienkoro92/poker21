...
function isClubAnalysisCommand(text) {
  return /^\/(?:разбор(?:\s+|_)клуба|club_analysis)(?:@[A-Za-z0-9_]+)?\s*$/iu.test(String(text || "").trim());
}
...
async function sendBoundActivity(chatId, binding, messageId = null) {
  const period = insightPeriods()[0];
  const owner = binding.type === "union" ? binding.league : binding.club;
  if (!period) {
    const sent = await telegram("sendMessage", { chat_id: chatId, text: `Для «${escapeTelegramHtml(owner)}» пока нет данных активности.`, parse_mode: "HTML" });
    return Boolean(sent.ok);
  }
...
function clubHistoryAnalysis(binding) {
  ...
  const periods = eligiblePeriods
    .filter((period) => (period.directory?.clubs || []).some((club) => String(club.id) === String(binding.clubId)))
    .slice()
    .reverse();
...
function clubAnalysisKeyboard() {
  return { inline_keyboard: [
    [{ text: "🧠 Разбор", callback_data: "pulse:analysis:overview" }, { text: "💎 Ядро", callback_data: "pulse:analysis:core" }],
    [{ text: "⏸ Неактивны сейчас", callback_data: "pulse:analysis:list:inactive:0" }],
    [{ text: "😴 Пропустили 1 отчёт", callback_data: "pulse:analysis:list:recent:0" }, { text: "💤 Нет 2+ отчёта", callback_data: "pulse:analysis:list:dormant:0" }],
    [{ text: "1️⃣ Один отчёт и сейчас неактивны", callback_data: "pulse:analysis:list:oneweek:0" }],
    [{ text: "🆕 Новички по неделям", callback_data: "pulse:analysis:cohorts" }],
    [{ text: "↩️ Главное меню", callback_data: "pulse:menu", style: "danger" }],
  ] };
}
...
async function sendClubAnalysisMenu(chatId, binding, messageId) {
  ...
  const lines = [
    `🧠 <b>Разбор клуба — ${escapeTelegramHtml(binding.club)}</b>`,
    ...
    ...(analysis.skippedPeriods ? [`⚠️ Не учтено отчётов без данных этого клуба: <b>${analysis.skippedPeriods}</b>`] : []),
    "",
  ...
  const sent = await telegram("editMessageText", {
    chat_id: chatId, message_id: messageId, text: lines.join("\n"), parse_mode: "HTML", reply_markup: clubAnalysisKeyboard(),
  });
  return Boolean(sent.ok);
}

function clubAnalysisBackKeyboard() {
  return { inline_keyboard: [[{ text: "↩️ К разбору", callback_data: "pulse:analysis" }], [{ text: "↩️ Главное меню", callback_data: "pulse:menu", style: "danger" }]] };
}

async function sendClubAnalysisOverview(chatId, binding, messageId) {
  const analysis = clubHistoryAnalysis(binding);
  if (!analysis.latest || !analysis.previous) {
    const sent = await telegram("editMessageText", { chat_id: chatId, message_id: messageId, text: "Для разбора нужны минимум два недельных отчёта.", reply_markup: clubAnalysisBackKeyboard() });
    return Boolean(sent.ok);
  }
...
async function sendClubCore(chatId, binding, messageId) {
  const analysis = clubHistoryAnalysis(binding);
  const lines = [
    `💎 <b>Ядро клуба — ${escapeTelegramHtml(binding.club)}</b>`,
    ...
    ...(analysis.skippedPeriods ? ["", `⚠️ Пропущено отчётов без данных клуба: ${analysis.skippedPeriods}.`] : []),
  ];
  const sent = await telegram("editMessageText", {
    chat_id: chatId,
    ...
    reply_markup: { inline_keyboard: [
      [{ text: "🟢 Стабильное ядро", callback_data: "pulse:analysis:list:stable:0" }],
      [{ text: "⭐ Звёзды", callback_data: "pulse:analysis:list:stars:0" }, { text: "🔴 Под угрозой", callback_data: "pulse:analysis:list:risk:0" }],
      [{ text: "🛡 Второй эшелон", callback_data: "pulse:analysis:list:support:0" }],
      [{ text: "📋 Всё экономическое ядро", callback_data: "pulse:analysis:list:core:0" }],
      ...clubAnalysisBackKeyboard().inline_keyboard,
    ] },
  });
  return Boolean(sent.ok);
}
...
async function sendClubAnalysisList(chatId, binding, kind, page, messageId) {
  ...
  const configs = {
    ...
    core: { title: "💎 Всё ядро клуба", rows: analysis.core },
    stable: { title: "🟢 Стабильное ядро", rows: analysis.stableCore },
    stars: { title: "⭐ Звёзды клуба", rows: analysis.stars },
  ...
  const navigation = [];
  if (safePage > 0) navigation.push({ text: "⬅️", callback_data: `pulse:analysis:list:${kind}:${safePage - 1}` });
  if ((safePage + 1) * pageSize < config.rows.length) navigation.push({ text: "➡️", callback_data: `pulse:analysis:list:${kind}:${safePage + 1}` });
  const lines = [
    `<b>${config.title} — ${escapeTelegramHtml(binding.club)}</b>`,
    `Всего: <b>${config.rows.length}</b>`,
    "",
    ...(rows.length ? rows.map((player, index) => `${safePage * pageSize + index + 1}. <b>${escapeTelegramHtml(player.nick)}</b> (${player.id})\nВсего рейка: ${formatRake(player.totalRake)} · активных отчётов: ${player.activeWeeks} · последний: ${displayIso(player.lastWeek.endDate)}`) : ["Игроков в этой категории нет."]),
  ];
  const keyboard = { inline_keyboard: [
    ...(navigation.length ? [navigation] : []),
    [{ text: "↩️ К разбору", callback_data: "pulse:analysis" }],
    [{ text: "↩️ Главное меню", callback_data: "pulse:menu", style: "danger" }],
  ] };
...
async function sendClubCohortsMenu(chatId, binding, messageId) {
  ...
  const rows = analysis.cohorts.map(({ period, total }, index) => [{
    ...
    callback_data: `pulse:analysis:cohort:${index}:0`,
  }]);
  rows.push([{ text: "↩️ К разбору", callback_data: "pulse:analysis" }]);
  rows.push([{ text: "↩️ Главное меню", callback_data: "pulse:menu", style: "danger" }]);
  const sent = await telegram("editMessageText", {
    chat_id: chatId,
    message_id: messageId,
    text: `🆕 <b>Когорты игроков — ${escapeTelegramHtml(binding.club)}</b>\n\nПервая доступная неделя — стартовая база. В следующих неделях показаны только игроки, которых раньше не было ни в одном загруженном отчёте клуба.`,
    parse_mode: "HTML",
...
async function sendClubCohortList(chatId, binding, cohortIndex, page, messageId) {
  ...
  const navigation = [];
  if (safePage > 0) navigation.push({ text: "⬅️", callback_data: `pulse:analysis:cohort:${cohortIndex}:${safePage - 1}` });
  if ((safePage + 1) * pageSize < cohortRows.length) navigation.push({ text: "➡️", callback_data: `pulse:analysis:cohort:${cohortIndex}:${safePage + 1}` });
  const lines = [
    ...
    "",
    ...(shown.length ? shown.map((player, index) => `${safePage * pageSize + index + 1}. ${player.currentActive ? "🟢" : "🔴"} <b>${escapeTelegramHtml(player.nick)}</b> (${player.id})\nВсего рейка: ${formatRake(player.totalRake)} · активных отчётов: ${player.activeWeeks} · последний: ${displayIso(player.lastWeek.endDate)}`) : ["Игроков нет."]),
  ];
  const keyboard = { inline_keyboard: [
    ...(navigation.length ? [navigation] : []),
    [{ text: "↩️ К неделям", callback_data: "pulse:analysis:cohorts" }],
    [{ text: "↩️ К разбору", callback_data: "pulse:analysis" }],
  ] };
  const sent = await telegram("editMessageText", { chat_id: chatId, message_id: messageId, text: lines.join("\n"), parse_mode: "HTML", reply_markup: keyboard });
  return Boolean(sent.ok);
...
async function sendInsightPlayers(chatId, binding, kind, editMessageId, returnToPulse = false) {
  ...
  const sent = await telegram("editMessageText", {
    ...
    reply_markup: returnToPulse
      ? { inline_keyboard: [[{ text: "↩️ Главное меню", callback_data: "pulse:menu", style: "danger" }]] }
...
function pulseMainKeyboard(binding) {
  const rows = [
    ...(binding?.type === "club" ? [[{ text: "🧠 Разбор клуба", callback_data: "pulse:analysis" }]] : []),
    binding?.type === "union"
      ? [
...
function pulsePlayersKeyboard(binding, selectedMetric = "", selectedMask = 1n) {
  ...
  return { inline_keyboard: [
    [
      ...
      { text: "📊 Активность", callback_data: "pulse:player:activity" },
    ],
    [
...
module.exports = async function handler(req, res) {
  ...
  const pulseAnalysisCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:analysis(?::(overview|core|cohorts))?$/) : null;
  const pulseAnalysisListCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:analysis:list:(inactive|recent|dormant|oneweek|new|core|stable|stars|risk|support):(\d{1,3})$/) : null;
  const pulseAnalysisCohortCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:analysis:cohort:(\d{1,3}):(\d{1,3})$/) : null;
  ...
  if (pulsePlayerSearchCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    const sent = await telegram("sendMessage", {
      chat_id: actorChatId,
      text: "🔎 <b>Поиск игрока</b>\n\nВведите ID или примерный ник игрока:",
      parse_mode: "HTML",
      reply_markup: { force_reply: true, selective: true, input_field_placeholder: "ID или ник игрока" },
    });
  ...
  if ((pulseMenuCallback || pulsePlayerCallback || pulsePlayerPeriodCallback || pulsePlayerSelectCallback || pulsePlayerToggleCallback || pulseTotalCallback || pulseWeeksCallback || pulseWeeksToggleCallback || pulseWeeksApplyCallback || pulseUnionClubsCallback || pulseUnionClubsSelectCallback || pulseUnionClubsToggleCallback || pulseUnionClubTopCallback || pulseUnionClubInfoCallback || pulseUnionClubInfoToggleCallback || pulseScheduleViewCallback || pulseAnalysisCallback || pulseAnalysisListCallback || pulseAnalysisCohortCallback) && callbackQuery?.message?.chat?.id != null) {
    ...
    if (pulseAnalysisCallback || pulseAnalysisListCallback || pulseAnalysisCohortCallback) {
      if (binding.type !== "club") {
        const sent = await sendPulseMainMenu(actorChatId, binding, null, callbackQuery.message.message_id);
        return res.status(200).json({ ok: true, pulseAnalysis: false, clubOnly: true, sent });
      }
      if (pulseAnalysisListCallback) {
        const sent = await sendClubAnalysisList(actorChatId, binding, pulseAnalysisListCallback[1], Number(pulseAnalysisListCallback[2]), callbackQuery.message.message_id);
      ...
      if (pulseAnalysisCohortCallback) {
        const sent = await sendClubCohortList(actorChatId, binding, Number(pulseAnalysisCohortCallback[1]), Number(pulseAnalysisCohortCallback[2]), callbackQuery.message.message_id);
      ...
      const sent = section === "overview"
        ? await sendClubAnalysisOverview(actorChatId, binding, callbackQuery.message.message_id)
        : section === "core"
          ? await sendClubCore(actorChatId, binding, callbackQuery.message.message_id)
          : section === "cohorts"
            ? await sendClubCohortsMenu(actorChatId, binding, callbackQuery.message.message_id)
    ...
    if (pulseMenuCallback) {
      ...
      if (section === "totals") {
        ...
        if (hasPhoto) {
          ...
          const sent = await telegram("sendMessage", {
            chat_id: actorChatId,
            text: `📊 <b>Отчёты — ${escapeTelegramHtml(binding.type === "union" ? binding.league : binding.club)}</b>\n\nКакой период показать?`,
            parse_mode: "HTML",
    ...
    if (pulseWeeksApplyCallback) {
      ...
      if (sent && hasPhoto) await telegram("deleteMessage", { chat_id: actorChatId, message_id: callbackQuery.message.message_id });
  ...
  if (/^🔎\s*Поиск игрока/iu.test(String(message.reply_to_message?.text || ""))) {
    ...
    const sent = binding.type === "union"
      ? await sendBoundUnionPlayerProfile(chatId, binding, query)
      : await sendBoundClubPlayerProfile(chatId, binding, query);
  ...
  if (clubBinding && (!isMainReportChat(chatId) || mainBalanceOnlyCommand)) {
    ...
    if (isClubAnalysisCommand(message.text)) {
      if (clubBinding.type !== "club") {
        const sent = await telegram("sendMessage", { chat_id: chatId, text: "Разбор клуба доступен в группе, привязанной к конкретному клубу." });
        return res.status(200).json({ ok: true, clubMode: true, clubAnalysis: false, clubOnly: true, sent: Boolean(sent.ok) });
      }
      const sent = await telegram("sendMessage", {
        chat_id: chatId,
        text: `🧠 <b>Разбор клуба — ${escapeTelegramHtml(clubBinding.club)}</b>\n\nОткрываю всю доступную историю игроков.`,
        parse_mode: "HTML",
        reply_to_message_id: message.message_id,
        reply_markup: clubAnalysisKeyboard(),
      });
