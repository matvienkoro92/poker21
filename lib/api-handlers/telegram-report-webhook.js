...
function pulseMainKeyboard(binding) {
  ...
  if (binding?.type === "union") rows.push([{ text: "🏢 Клубы союза", callback_data: "pulse:clubs" }]);
...
async function sendBoundClubCommands(chatId, binding) {
  if (binding.type === "union") {
    const lines = [
      ...
      "",
      "<b>Статистика</b>",
      "<b>/пульс</b> — динамика, игроки, итоги и клубы союза",
...
module.exports = async function handler(req, res) {
  ...
  const pulseMenuCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:(menu|dynamics|players|totals|clubs)$/) : null;
