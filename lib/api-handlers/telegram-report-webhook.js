...
async function getClubBinding(chatId) {
  ...
  return clubChatBindings.get(String(chatId)) || null;
}
...
function isCurrentClubCommand(text) {
  return /^\/(?:мой\s+(?:клуб|союз)|клуб|союз|club|union|отчет|отчёт|report)(?:@[A-Za-z0-9_]+)?\s*$/iu.test(String(text || "").trim());
}
...
async function sendBoundClubCommands(chatId, binding) {
  if (binding.type === "union") {
    const lines = [
      ...
      "",
      "<b>/мой союз</b> — текущий отчёт союза",
      "<b>/отчет</b> — текущий отчёт союза",
      "<b>/клубы</b> — клубы союза и их рейк за последнюю неделю",
      "<b>/клубы 03.08-09.08</b> — клубы и рейк за выбранный период",
...
module.exports = async function handler(req, res) {
  ...
  if (clubBinding) {
    ...
    if (boundUnionSelection && isClubsCommand(boundUnionSelection.text)) {
