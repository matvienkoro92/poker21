...
async function sendTotal(chatId, messageId, command, servicePercent) {
  ...
  const mttFeeTotal = Math.round(feeBreakdown
    .filter((row) => /^MTT-/i.test(row.name))
  ...
  const lines = [
    ...
    ...feeBreakdown.map((row) => `${row.name}: ${formatRub(row.value)}`),
    `Комиссия MTT: ${formatRub(mttFeeTotal)}`,
    `Страховка: ${formatRub(metricTotals.insurance)}`,
    "",
    `<b>Итого Рейк + выигрыш: ${formatRub(metricTotals.balanceApp)}</b>`,
    ...
    "",
    `<b>Итого к расчёту: ${formatRub(total)}</b>`,
  ];
