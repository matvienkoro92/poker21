...
function isPaymentNotificationsCommand(text) {
  return /^\/уведомления(?:@[A-Za-z0-9_]+)?\s*$/iu.test(String(text || "").trim());
}
...
async function sendPaymentDetailsRegistry(chatId, allowActions = true) {
  ...
  active.forEach((item, index) => {
    ...
    lines.push(`${index + 1}. ${marker} <b>${formatPaymentAmount(item)}</b> · ${escapeTelegramHtml(item.owner.name)} · ${escapeTelegramHtml(bank)}`);
  });
  const actionButtons = active.map((item, index) => ({ text: `${index + 1} · ${formatPaymentAmount(item)}`, callback_data: `payreq:view:${item.id}` }));
  const keyboard = [];
...
async function sendAllChatBalances(chatId, messageId) {
  ...
  const unrecordedResult = await redisPipeline(
    [["LRANGE", UNRECORDED_BALANCE_OPERATIONS_KEY, "0", "-1"]],
    { context: "telegram-report.balances.unrecorded", timeoutMs: 4000 },
  );
  const unrecorded = Array.isArray(unrecordedResult?.[0]?.result)
    ? unrecordedResult[0].result.map((raw) => { try { return JSON.parse(String(raw)); } catch (_) { return null; } }).filter(Boolean)
    : [];
  ...
  const sent = await telegram("sendMessage", {
    chat_id: chatId,
    text: ["<b>Балансы союзов</b>", "", ...unionLines, "", "<b>Балансы клубов</b>", "", ...clubLines, "", "<b>Не записано</b>", "", ...unrecordedLines].join("\n"),
    parse_mode: "HTML",
    reply_to_message_id: messageId,
  });
  return Boolean(sent.ok);
...
module.exports = async function handler(req, res) {
  ...
  const paymentNotificationCallback = callbackQuery ? String(callbackQuery.data || "").match(/^paynotify:set:(under5000|from5000|all)$/) : null;
  const paymentCallback = callbackQuery ? String(callbackQuery.data || "").match(/^payreq:(view|list|take|paid|cancel|confirm|reject):([A-Za-z0-9_-]+)$/) : null;
  if (paymentNotificationCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    await telegram("sendMessage", {
      chat_id: actorChatId,
      text: saved?.[0]?.result === "OK" ? `Уведомления включены: ${labels[preference]}.` : "Не удалось сохранить настройку уведомлений.",
    });
    return res.status(200).json({ ok: true, paymentNotifications: saved?.[0]?.result === "OK", preference });
  }
  if (paymentCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    if (action === "list") {
      const sent = await sendPaymentDetailsRegistry(actorChatId, !isReadOnlyMainRegistry);
  ...
  if (hasReceiptAttachment && receiptMessage.chat?.id != null) {
    ...
    if (item) {
      ...
      await savePaymentDetails(item);
      await notifyPaymentParty(payerChatId, `Чек отправлен получателю. Оплата <b>${formatPaymentAmount(item)}</b> ожидает подтверждения.`);
