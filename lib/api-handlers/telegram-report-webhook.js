...
function pulseMainKeyboard(binding, balance) {
  const rows = [
  ...
  rows.push([{ text: "💳 Реквизиты", callback_data: "paymenu:list", style: "success" }]);
...
async function sendPulseMainMenu(chatId, binding, messageId = null, editMessageId = null) {
  ...
  const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", {
    ...
    parse_mode: "HTML",
    reply_markup: pulseMainKeyboard(binding, balance),
  });
  return Boolean(sent.ok);
}
