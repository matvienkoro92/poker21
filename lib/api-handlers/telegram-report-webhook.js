...
function parseBalanceCommand(text) {
  ...
  const currencyCount = valueMatches.length;
  let rub = null;
  let usd = null;
  ...
  if (clubBinding && (!isMainReportChat(chatId) || mainBalanceOnlyCommand)) {
    ...
    if (balanceCommand) {
      ...
      if (balanceCommand.action === "invalid") {
        const sent = await telegram("sendMessage", { chat_id: chatId, text: "Укажите валюту: /баланс 300р, 40$, /баланс +50$ или /баланс -100р. Комментарий можно написать после суммы: /баланс -79000р комментарий" });
