...
async function sendOverview(chatId, data = latestUnionData) {
  ...
  const lines = [
    "<b>СВОДКА</b>",
    "",
    `1. Доля разработчика (китайцев): <b>${formatRake(share(60))}</b>`,
    `2. Наша доля: <b>${formatRake(share(40))}</b>`,
    `3. Джекпоты: <b>${formatRake(jackpotNet)}</b>`,
    `4. Клубы нашего союза (Anti-Reg): <b>${formatRake(antiRegClubs)}</b>`,
    `5. Другие союзы без Anti-Reg: <b>${formatRake(otherUnions)}</b>`,
    `6. Откаты: <b>+${formatRake(kickbacks)}</b>`,
    `7. Оверлей: <b>-${formatRake(overlay)}</b>`,
    `8. ЗП: <b>+${formatRake(salaries)}</b>`,
    "",
