...
const SETTINGS_HASH_KEY = "poker_app:telegram_report:service_percent";
const SETTINGS_PENDING_TTL_SECONDS = 10 * 60;

function pendingSettingsKey(chatId, userId) {
  return `poker_app:telegram_report:settings_pending:${chatId}:${userId}`;
}
...
async function redisPipeline(commands) {
  ...
  try {
    ...
    console.error("telegram-report-webhook: Redis request failed", error && error.message ? error.message : error);
    return null;
  }
}

function parseSettingsCommand(text) {
  return /^\/(?:настройка|settings)(?:@[A-Za-z0-9_]+)?$/iu.test(String(text || "").trim());
}

function parseServicePercent(text) {
  const normalized = String(text || "").trim().replace(/\s+/g, "").replace(/%$/, "").replace(",", ".");
...
async function getServicePercent(chatId) {
  const result = await redisPipeline([["HGET", SETTINGS_HASH_KEY, String(chatId)]]);
  const raw = result && result[0] ? result[0].result : null;
  const value = Number(raw);
  return raw !== null && raw !== "" && Number.isFinite(value) && value >= 0 && value <= 100
    ? value
...
module.exports = async function handler(req, res) {
  ...
  if (parseSettingsCommand(message.text)) {
    if (!userId) return res.status(200).json({ ok: true, settings: false });
    const saved = await redisPipeline([["SETEX", pendingSettingsKey(chatId, userId), String(SETTINGS_PENDING_TTL_SECONDS), "service_percent"]]);
    const text = saved
      ? "Введите процент за обслуживание (от 0 до 100). Например: 8 или 8,5%."
      : "Не удалось открыть настройку. Попробуйте ещё раз позже.";
    await telegram("sendMessage", { chat_id: chatId, text, reply_to_message_id: message.message_id });
    return res.status(200).json({ ok: true, settings: Boolean(saved) });
  }

  if (userId) {
    const pendingKey = pendingSettingsKey(chatId, userId);
    const pendingResult = await redisPipeline([["GET", pendingKey]]);
    const pending = pendingResult && pendingResult[0] ? pendingResult[0].result : null;
    if (pending === "service_percent") {
      const servicePercent = parseServicePercent(message.text);
      if (servicePercent === null) {
        await telegram("sendMessage", {
          chat_id: chatId,
          text: "Введите число от 0 до 100. Можно использовать запятую и знак %, например: 8,5%.",
          reply_to_message_id: message.message_id,
        });
        return res.status(200).json({ ok: true, settings: true, saved: false });
      }
      const saved = await redisPipeline([
        ["HSET", SETTINGS_HASH_KEY, chatId, String(servicePercent)],
        ["DEL", pendingKey],
      ]);
      const text = saved
        ? `Процент за обслуживание изменён на ${formatPercent(servicePercent)}%.`
        : "Не удалось сохранить процент. Попробуйте ещё раз позже.";
      await telegram("sendMessage", { chat_id: chatId, text, reply_to_message_id: message.message_id });
      return res.status(200).json({ ok: true, settings: true, saved: Boolean(saved), servicePercent });
    }
  }

  const totalCommand = parseTotalCommand(message.text);
  if (totalCommand) {
    const servicePercent = await getServicePercent(chatId);
  ...
  const sent = await telegram("sendPhoto", {
    ...
    caption: `Отчёт клуба «${report.club}»\nПериод: ${periodText}\nОбслуживание: ${formatPercent(servicePercent)}% (${formatRub(report.metrics.service)})\nИтого к расчёту: ${formatRub(report.total)}`,
    reply_to_message_id: message.message_id,
  });
  if (!sent.ok) {
  ...
  const documentSent = report.excelPath
    ? await telegram("sendDocument", {
        chat_id: chatId,
        document: encodeURI(`${APP_ORIGIN}${report.excelPath}`),
