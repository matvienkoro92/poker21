...
function claimClubBroadcast(update, message) {
...
function getClubReportGroups(data = latestUnionData) {
  ...
  const romanClubs = new Set(["GARAGE", "GoRiLaZzz", "PC Arena", "RealPokerGame", "River21", "Sibir 70", "Два Туза", "РИВЕР КЛУБ", "T O T", "Храм"]);
  return {
    reports,
    groups: [
      { heading: "❗ ДЛЯ РОМАНА:", recipient: "Роман", reports: reports.filter((report) => romanClubs.has(report.club)) },
      { heading: "❗ ДЛЯ СЕРГЕЯ:", recipient: "Сергей", reports: reports.filter((report) => !romanClubs.has(report.club)) },
    ],
  };
}
...
async function sendClubReports(chatId, messageId, data = latestUnionData) {
  ...
  for (const [groupIndex, group] of groups.entries()) {
    ...
    const media = group.reports.map((report) => {
      ...
      return { type: "photo", media: `${APP_ORIGIN}${report.imagePath}?v=club-reports-1`, caption: lines.join("\n"), parse_mode: "HTML" };
    });
...
module.exports = async function handler(req, res) {
  ...
  if (isClubsCommand(unionText)) {
    if (!claimClubBroadcast(update, message)) {
      return res.status(200).json({ ok: true, clubs: true, duplicate: true });
    }
    res.status(200).json({ ok: true, clubs: true, accepted: true });
    await sendClubReports(chatId, message.message_id, unionData);
    return res;
  }
