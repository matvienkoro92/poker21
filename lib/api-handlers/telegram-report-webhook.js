...
async function sendPaymentDetailsRegistry(chatId, allowActions = true) {
  ...
  if (!active.length) {
    const sent = await telegram("sendMessage", {
      chat_id: chatId,
      text: "Активных реквизитов пока нет.\n\nДля размещения: /реквизиты 50000р | Сбербанк, телефон, получатель",
    });
    return Boolean(sent.ok);
  }
  ...
  if (clubBinding) {
    ...
    if (paymentDetailsCommand) {
      ...
      if (paymentDetailsCommand.action === "invalid") {
        const sent = await telegram("sendMessage", {
          chat_id: chatId,
          text: "Формат размещения:\n/реквизиты 50000р | Сбербанк, +7…, Иван И.\n/реквизиты 100$ | USDT TRC20, адрес",
        });
