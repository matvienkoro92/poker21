...
function paymentRegistryButton(item, index, chatId) {
  ...
  const status = inWork ? " · Взят в работу" : awaiting ? " · Ждёт подтверждения" : "";
  return { text: `${inWork ? "🟡 " : awaiting ? "🔵 " : ""}${ownMark}${index + 1} · ${formatPaymentAmount(item)}${status}`, callback_data: `payreq:view:${item.id}` };
}
...
module.exports = async function handler(req, res) {
  ...
  if (callbackQuery?.data === "pulse:player:activity" && callbackQuery.message?.chat?.id != null) {
    ...
    const sent = binding ? await sendBoundActivity(chatId, binding, null, callbackQuery.message.message_id) : false;
