...
async function getPaymentBalanceHistory(chatId, limit = 3) {
  ...
  const transfers = await redisPipeline([["LRANGE", `poker21:telegram-report:payment-merge-history:${chatId}`, "0", "-1"]], { context: "payment-history.merges", timeoutMs: 2000 });
  for (const raw of Array.isArray(transfers?.[0]?.result) ? transfers[0].result : []) {
