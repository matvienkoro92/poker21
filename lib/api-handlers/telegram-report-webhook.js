...
const POKER21_TELEGRAM_APP_URL = "https://t.me/poker21app_bot/start";
...
function isPosterCommand(text) {
  return /^\/(?:афиша|poster)(?:@[A-Za-z0-9_]+)?\s*$/iu.test(String(text || "").trim());
}
...
async function tournamentPosterData(scheduleChatId) {
  ...
  const lines = [
    `🎰 Турниры Poker21 на сегодня — ${formattedToday}`,
    "",
    ...(todayRows.length
      ? todayRows.map((row) => String(row.text || "").trim()).filter(Boolean)
      : ["На сегодня турниры не указаны."]),
    "",
    "Заходи в Poker21 и выбирай свой турнир! 🔥",
  ];
  return {
    posterUrl: schedulePosterUrl("today", dateParts),
    text: lines.join("\n"),
...
function posterKeyboard() {
  return {
    inline_keyboard: [
      [{ text: "📋 Получить текст", callback_data: "poster:text" }],
      [
        { text: "🖼 Скачать афишу", callback_data: "poster:download" },
        { text: "🎮 Открыть Poker21", url: POKER21_TELEGRAM_APP_URL },
      ],
    ],
  };
}

function posterXml(value) {
  return String(value || "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function posterWrappedLines(value, maxLength = 43) {
  const result = [];
  for (const paragraph of String(value || "").replace(/\s+/g, " ").trim().split(/\n+/)) {
    let line = "";
...
async function renderTournamentPoster(poster) {
  const width = 1080;
  const height = 1350;
  const rows = poster.rows || [];
  const visualLines = [];
  rows.forEach((row, index) => {
    const wrapped = posterWrappedLines(String(row.text || "").replace(/\r?\n/g, " · "));
    visualLines.push({ text: `${index + 1}. ${wrapped.shift() || "Турнир"}`, first: true });
    wrapped.forEach((text) => visualLines.push({ text, first: false }));
  });
  const limited = visualLines.slice(0, 16);
  if (visualLines.length > limited.length) limited.push({ text: `И ещё турниров: ${rows.length}`, first: false });
  if (!limited.length) limited.push({ text: "На сегодня турниры не указаны", first: true });
  const startY = 365;
  const lineHeight = Math.min(63, Math.max(42, Math.floor(780 / Math.max(1, limited.length))));
  const listSvg = limited.map((line, index) => (
    `<text x="${line.first ? 105 : 145}" y="${startY + index * lineHeight}" fill="${line.first ? "#ffffff" : "#d7dbea"}" font-family="Arial, Helvetica, sans-serif" font-size="${line.first ? 36 : 31}" font-weight="${line.first ? 700 : 400}">${posterXml(line.text)}</text>`
  )).join("");
    ...
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1">
          ...
          <stop offset="0.52" stop-color="#241451"/>
          <stop offset="1" stop-color="#691d53"/>
        </linearGradient>
        <radialGradient id="glow"><stop offset="0" stop-color="#ffcf5b" stop-opacity=".42"/><stop offset="1" stop-color="#ffcf5b" stop-opacity="0"/></radialGradient>
      </defs>
      <rect width="1080" height="1350" fill="url(#bg)"/>
      <circle cx="900" cy="120" r="430" fill="url(#glow)"/>
      <circle cx="65" cy="1180" r="350" fill="#7846ff" opacity=".14"/>
      <rect x="58" y="55" width="964" height="1240" rx="42" fill="#090d1d" opacity=".48" stroke="#ffffff" stroke-opacity=".15" stroke-width="2"/>
      <text x="540" y="145" text-anchor="middle" fill="#ffcf5b" font-family="Arial, Helvetica, sans-serif" font-size="42" font-weight="700">POKER21</text>
      <text x="540" y="230" text-anchor="middle" fill="#ffffff" font-family="Arial, Helvetica, sans-serif" font-size="68" font-weight="800">ТУРНИРЫ НА СЕГОДНЯ</text>
      <text x="540" y="292" text-anchor="middle" fill="#c9cce0" font-family="Arial, Helvetica, sans-serif" font-size="33">${posterXml(String(poster.formattedToday || "").toLocaleUpperCase("ru-RU"))} · МСК</text>
      <line x1="105" y1="326" x2="975" y2="326" stroke="#ffcf5b" stroke-width="3" opacity=".7"/>
      ${listSvg}
      <rect x="145" y="1190" width="790" height="78" rx="39" fill="#ffcf5b"/>
      <text x="540" y="1242" text-anchor="middle" fill="#181126" font-family="Arial, Helvetica, sans-serif" font-size="34" font-weight="800">ВЫБИРАЙ ТУРНИР И ЗАХОДИ В ИГРУ</text>
    </svg>`);
  return sharp(svg).jpeg({ quality: 91, mozjpeg: true }).toBuffer();
}

async function sendTournamentPosterFile(method, chatId, poster, options = {}) {
  const image = await renderTournamentPoster(poster);
  const form = new FormData();
  form.append("chat_id", String(chatId));
  const field = method === "sendDocument" ? "document" : "photo";
  form.append(field, new Blob([image], { type: "image/jpeg" }), "poker21-today.jpg");
...
async function sendTournamentPoster(chatId, messageId, scheduleChatId = chatId) {
  const poster = await tournamentPosterData(scheduleChatId);
  const maxCaptionLength = 1000;
  const caption = poster.text.length <= maxCaptionLength
    ? poster.text
    : `${poster.text.slice(0, maxCaptionLength - 1).trimEnd()}…`;
  const sent = await sendTournamentPosterFile("sendPhoto", chatId, poster, {
    caption,
    replyToMessageId: messageId,
    replyMarkup: posterKeyboard(),
  });
...
async function sendCommands(chatId, messageId) {
  ...
  const lines = [
    ...
    "<b>/афиша</b> — готовая картинка и текст для игроков",
...
module.exports = async function handler(req, res) {
  ...
  const posterCallback = callbackQuery ? String(callbackQuery.data || "").match(/^poster:(text|download)$/) : null;
  ...
  if (posterCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    const poster = await tournamentPosterData(publicScheduleChat ? ANTIREG_REPORT_CHAT_ID : actorChatId);
    const action = posterCallback[1];
    await telegram("answerCallbackQuery", { callback_query_id: callbackQuery.id, text: action === "text" ? "Текст готов" : "Готовлю файл" });
    if (action === "download") {
      const sent = await sendTournamentPosterFile("sendDocument", actorChatId, poster, {
        caption: "Афиша Poker21 на сегодня",
        replyToMessageId: callbackQuery.message.message_id,
      });
      return res.status(200).json({ ok: true, posterDownload: true, sent: Boolean(sent.ok) });
    }
    const sent = await telegram("sendMessage", {
      chat_id: actorChatId,
      text: poster.text,
    ...
    return res.status(200).json({ ok: true, posterText: true, sent: Boolean(sent.ok) });
  }
  ...
  if (publicScheduleChat && !isScheduleCommand(message.text) && !isPosterCommand(message.text)) {
  ...
  if (isPosterCommand(message.text)) {
    if (!isRedisConfigured()) {
      const sent = await telegram("sendMessage", { chat_id: chatId, text: "Афиша недоступна: расписание не настроено." });
      return res.status(200).json({ ok: true, poster: false, sent: Boolean(sent.ok) });
    }
    const sent = await sendTournamentPoster(
      chatId,
      message.message_id,
      publicScheduleChat ? ANTIREG_REPORT_CHAT_ID : chatId,
    );
    return res.status(200).json({ ok: true, poster: true, sent });
  }
