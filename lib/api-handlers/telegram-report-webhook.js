...
async function sendChineseRake(chatId, messageId, data = latestUnionData) {
  ...
  const rows = CHINESE_RAKE_RULES.map((rule) => {
    ...
    const amount = Math.round((rake * rule.percent / 100) * 100) / 100;
    return { ...rule, rake, amount };
  });
  const total = Math.round(rows.reduce((sum, row) => sum + row.amount, 0) * 100) / 100;
  const share = (percent) => Math.round((total * percent / 100) * 100) / 100;
  const lines = [
    "<b>Китайцы</b>",
    ...rows.map((row) => `${formatPercent(row.percent)}% — ${escapeTelegramHtml(row.label)} — общий рейк: ${formatRake(row.rake)} — процент: ${formatRake(row.amount)}`),
    "",
    `<b>ИТОГО: ${formatRake(total)}</b>`,
    "",
...
async function sendCommands(chatId, messageId) {
  const lines = [
    ...
    "<b>/китайцы</b> — рейк союзов и начисленный процент",
...
module.exports = async function handler(req, res) {
  ...
  if (isChineseCommand(unionText)) {
    const sent = await sendChineseRake(chatId, message.message_id, unionData);
