...
const MAIN_REPORT_CHAT_ID = "-1004391487736";
...
async function sendCommands(chatId, messageId) {
  const lines = [
    ...
    ...(String(chatId) === MAIN_REPORT_CHAT_ID ? ["<b>/балансы</b> — текущие балансы союзов и клубов"] : []),
...
async function sendChatBalance(chatId) {
  ...
  if (balance.history.length) {
    lines.push("", "Последние изменения:", ...balance.history.map((entry) => {
      ...
      return `${timestamp ? `${timestamp} — ` : ""}${changes.join(", ")} — ${escapeTelegramHtml(entry.actor || "администратор")}`;
    }));
  }
  const sent = await telegram("sendMessage", { chat_id: chatId, text: lines.join("\n"), parse_mode: "HTML" });
  return Boolean(sent.ok);
...
async function sendAllChatBalances(chatId, messageId) {
  ...
  const keysResult = await redisPipeline([["KEYS", "poker21:telegram-report:club-chat:*"]], { context: "telegram-report.balances.keys", timeoutMs: 4000 });
  const keys = Array.isArray(keysResult?.[0]?.result) ? keysResult[0].result : [];
  ...
  const balanceFor = (target, type) => {
    ...
    return formatStoredBalance(selected?.balance);
  };
  const unionLines = BALANCE_UNION_ROWS.map((row) => {
    ...
    return `${label} — ${balanceFor(row, "union")}`;
  });
  const clubLines = BALANCE_CLUB_ROWS.map((row) => `${escapeTelegramHtml(row.label)} — ${balanceFor(row, "club")}`);
  const sent = await telegram("sendMessage", {
    chat_id: chatId,
