...
module.exports = async function handler(req, res) {
  ...
  if (/^🔎\s*Поиск клуба/iu.test(String(message.reply_to_message?.text || ""))) {
    ...
    const exactId = clubs.find((club) => String(club.clubId || club.id || "") === query);
    const matches = exactId ? [exactId] : clubs
    ...
    if (!matches.length) {
      const sent = await telegram("editMessageText", {
        ...
        text: `Клуб «${escapeTelegramHtml(query)}» в союзе «${escapeTelegramHtml(binding.league)}» не найден.`,
        parse_mode: "HTML",
    ...
    if (matches.length > 1 && lookupScore(matches[0].club || matches[0].name, query) === lookupScore(matches[1].club || matches[1].name, query)) {
      const sent = await telegram("editMessageText", {
        ...
        reply_markup: { inline_keyboard: [
          ...matches.map((club) => [{
            text: String(club.club || club.name || club.clubId),
            callback_data: `pulse:unionclubs:club:${club.clubId || club.id}:${mask.toString(16)}`,
          }]),
