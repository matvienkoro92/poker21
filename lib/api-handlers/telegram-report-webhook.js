...
function pulseTotalsKeyboard(binding) {
  return { inline_keyboard: [
    ...(binding.type === "club" ? [
      [{ text: "Последние недели", callback_data: "pulse:total:choose" }],
      [{ text: "Прошлый месяц", callback_data: "pulse:total:month" }],
...
function availableConsecutiveReportWeeks(binding) {
  if (binding.type !== "club") return 0;
  const endDates = [...new Set((reportIndex.reports || [])
    .filter((report) => String(report.clubId) === String(binding.clubId))
    .map((report) => String(report.endDate || ""))
    .filter(Boolean))]
    .sort((a, b) => b.localeCompare(a));
  if (!endDates.length) return 0;
  let count = 1;
  for (let index = 1; index < endDates.length; index += 1) {
    const newer = Date.parse(`${endDates[index - 1]}T00:00:00Z`);
    const older = Date.parse(`${endDates[index]}T00:00:00Z`);
...
async function sendPulseWeeksMenu(chatId, binding, messageId) {
  const count = availableConsecutiveReportWeeks(binding);
  const sent = await telegram("editMessageText", {
    chat_id: chatId,
    message_id: messageId,
    text: count
      ? `За сколько последних недель показать итог?\n\nДоступно последовательных отчётов: ${count}.`
      : "Для этого клуба пока нет последовательных недельных отчётов.",
    reply_markup: pulseWeeksKeyboard(binding),
  });
  return Boolean(sent.ok);
}
...
function reportsCarouselKeyboard(index, count) {
  if (!Number.isInteger(index) || !Number.isInteger(count) || count < 2) return undefined;
  const row = [];
  if (index < count - 1) row.push({ text: "⬅️ Старее", callback_data: `reports:period:${index + 1}` });
  if (index > 0) row.push({ text: "Новее ➡️", callback_data: `reports:period:${index - 1}` });
...
async function sendCurrentBoundClub(chatId, binding, data = latestUnionData, options = {}) {
  ...
  const carouselMarkup = reportsCarouselKeyboard(options.periodIndex, options.periodCount);
  if (binding.type === "union") {
...
module.exports = async function handler(req, res) {
  ...
  if ((pulseMenuCallback || pulsePlayerCallback || pulseTotalCallback || pulseWeeksCallback) && callbackQuery?.message?.chat?.id != null) {
    const actorChatId = String(callbackQuery.message.chat.id);
    await telegram("answerCallbackQuery", { callback_query_id: callbackQuery.id });
    const binding = await getClubBinding(actorChatId);
    if (!binding) {
