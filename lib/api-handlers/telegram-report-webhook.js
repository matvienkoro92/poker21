...
async function sendCalculations(chatId, messageId, data = latestUnionData) {
  ...
  const formula = `${formatRake(winLose)} + ${formatRake4(fee)} + ${formatRake(insurance)} + ${formatRake(jackpotFee)} - ${formatRake(jackpotPayout)} - ${formatRake(overlay)}`;
  const lines = [
    "Расчёты суперюниона",
    ...
    `Insurance всех лиг +<b>${formatRake(insurance)}</b>`,
...
async function sendCommands(chatId, messageId) {
  const lines = [
    ...
    "<b>/расчеты</b> — win/lose, fee, insurance, джекпот, оверлей, формула и итог",
...
module.exports = async function handler(req, res) {
  ...
  const isUnionCommand = isActivityCommand(unionText) || isSummaryCommand(unionText) || isGamesCommand(unionText) ||
    isCalculationsCommand(unionText) ||
