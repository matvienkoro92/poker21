...
module.exports = async function handler(req, res) {
  ...
  if (clubBinding && isMainReportChat(chatId)) {
  ...
  if (clubBinding && !isMainReportChat(chatId)) {
    ...
    if (autoReportCommand) {
      if (String(clubBinding.clubId) === "964699" && autoReportCommand === "вкл") {
        const sent = await telegram("sendMessage", { chat_id: chatId, text: "Автоотчёт для Kings KO отключён в настройках главной бухгалтерии." });
