...
function pulseTotalsKeyboard(binding) {
  return { inline_keyboard: [
    ...(binding.type === "club" ? [
      [{ text: "3 недели", callback_data: "pulse:total:weeks" }],
...
async function sendBindingPulse(chatId, binding, messageId, mode = "week", editMessageId = null) {
  const periods = insightPeriods();
  const latest = periods[0];
  const previous = mode === "month" ? periods[3] : periods[1];
  if (!latest || !previous) {
    const sent = await telegram("sendMessage", {
      chat_id: chatId,
      text: mode === "month" ? "Для пульса за месяц нужны отчёты минимум за четыре недели." : "Для пульса нужны как минимум два недельных отчёта.",
    });
    return Boolean(sent.ok);
  }
  const current = insightPulseMetrics(latest, binding);
  const prior = insightPulseMetrics(previous, binding);
  const owner = binding.type === "union" ? binding.league : binding.club;
  const lines = [
    `<b>💓 Пульс ${binding.type === "union" ? "союза" : "клуба"} — ${escapeTelegramHtml(owner)}</b>`,
    `<b>Период: ${displayIso(latest.startDate)}–${displayIso(latest.endDate)}</b>`,
    `<b>${mode === "month" ? "Динамика за месяц" : "Динамика за неделю"}</b>`,
    `Сравнение с ${displayIso(previous.startDate)}–${displayIso(previous.endDate)}`,
    "",
...
module.exports = async function handler(req, res) {
  ...
  const clubInsightCallback = callbackQuery ? String(callbackQuery.data || "").match(/^clubinsight:(new|sleeping|returned|stable)$/) : null;
  ...
  const pulseTotalCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:total:(weeks|month|all|union)$/) : null;
  ...
  if ((pulseMenuCallback || pulsePlayerCallback || pulseTotalCallback) && callbackQuery?.message?.chat?.id != null) {
    const actorChatId = String(callbackQuery.message.chat.id);
    await telegram("answerCallbackQuery", { callback_query_id: callbackQuery.id });
    const binding = await getClubBinding(actorChatId);
    if (!binding) {
    ...
    const totalType = pulseTotalCallback[1];
    if (binding.type === "union") {
    ...
    const command = totalType === "weeks" ? { type: "weeks", count: 3 }
