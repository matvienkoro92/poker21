...
const { balanceButtonText, trackMenu, refreshMenu } = require("../pulse-balance-menu");
...
function parsePaymentDetailsMessage(text, awaitingInput = false) {
  ...
  return command?.action === "publish" || awaitingInput ? command : null;
}
...
function paymentDetailsFormText(error = false) {
  return [
    ...
    "Имя получателя",
    "",
    "Отправьте обычным сообщением. Отвечать на это сообщение не обязательно.",
    "",
    "Например:",
    "5000",
    "+7 999 999-99-99",
...
function pulseMainKeyboard(binding, balance, requisitesCount = null) {
  ...
  rows.push([{ text: `💳 Реквизиты${requisitesCount == null ? "" : ` — ${requisitesCount}`}`, callback_data: "paymenu:list", style: "success" }]);
...
async function routePulseRootCallback(callback) {
  ...
  if (!message?.text?.startsWith("❤️ Пульс ") || !message.reply_markup?.inline_keyboard?.flat().some(button => button.callback_data === "pulse:balance")) return callback;
  const chatId = String(message.chat.id);
  const key = `poker21:telegram-report:pulse-workspace:${chatId}`;
  const stored = isRedisConfigured() ? await redisPipeline([["GET", key]], { context: "pulse.workspace.get", timeoutMs: 2000 }) : null;
  let workId = Number(stored?.[0]?.result) || null;
  if (workId === message.message_id) return callback;
  let result = workId ? await telegram("editMessageText", { chat_id: chatId, message_id: workId, text: "Открываю раздел…", reply_markup: { inline_keyboard: [] } }) : null;
  if (!result?.ok && !String(result?.description || "").includes("message is not modified")) {
    result = await telegram("sendMessage", { chat_id: chatId, text: "Открываю раздел…" });
    workId = result.result?.message_id;
    if (!result.ok || !workId) return null;
    if (isRedisConfigured()) await redisPipeline([["SET", key, String(workId)]], { context: "pulse.workspace.save", timeoutMs: 2000 });
  }
  return { ...callback, message: { ...message, message_id: workId, text: "Открываю раздел…", reply_markup: { inline_keyboard: [] } } };
}
...
async function sendPaymentPlacementMessage(chatId, userId, text, buttons = [], preferredMessageId = null) {
...
async function sendAllPaymentBalances(chatId, messageId, requisites = false) {
  ...
  const lines = [
    ...
    ...(requisites ? ["Только подтверждённые оплаты. Новые операции: плательщику +99%, владельцу реквизитов −101% суммы (вычет 1% у каждого). Старые операции не пересчитываются. Рубли и доллары учитываются отдельно."] : []),
    "",
    "<b>Союзы</b>",
    "",
...
module.exports = async function handler(req, res) {
  ...
  if (paymentCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    const balanceResult = await redisPipeline([
      ["INCRBY", ownerBalanceKey, String(deltas.ownerDeltaCents)],
      ["INCRBY", payerBalanceKey, String(deltas.payerDeltaCents)],
    ...
    await sendOrEditPaymentMessage(actorChatId, [
      ...
      `Итого изменение: ${formatBalanceAmount(deltas.ownerDeltaCents, symbol)}`,
    ].join("\n"), [], callbackQuery.message.message_id);
    await notifyPaymentParty(item.payer.chatId, [
      ...
      `Операция: +${formatPaymentAmount(item)}`,
      `Вычет 1%: −${formatPaymentAmount({ ...item, amountCents: deltas.feeCents })}`,
      `Итого изменение: ${formatBalanceAmount(deltas.payerDeltaCents, symbol)}`,
    ].join("\n"));
    return res.status(200).json({ ok: true, paymentDetails: true, action });
  }
  ...
  if (clubBinding && (!isMainReportChat(chatId) || mainBalanceOnlyCommand)) {
    ...
    if (paymentDetailsCommand) {
      ...
      if (paymentDetailsCommand.action === "invalid") {
        ...
        const sent = await sendPaymentPlacementMessage(chatId, message.from?.id, paymentDetailsFormText(true));
