...
async function sendJackpot(chatId, messageId, data = latestUnionData) {
  ...
  const leagues = (Array.isArray(unionJackpotSummary.leagues) ? unionJackpotSummary.leagues : []).map((row) => ({
    ...row,
    fee: Number(row.fee || 0) * (String(row.league || "").toLowerCase() === "ppcunion" ? 100 : 1),
  }));
  ...
  const refundRules = [
    ...
    { label: "AQUARIUM", league: "AQUARIUM", percent: 50, exchangeRate: 100 },
  ];
  ...
  const finalNet = net - totalRefunds;
  const lines = [
    "Джекпот суперюниона",
  ...
  lines.push(
    ...
    "",
    `<b>Полный итог: ${formatRake(finalNet)}</b>`,
  );
  const sent = await telegram("sendMessage", {
    chat_id: chatId,
    text: lines.join("\n"),
