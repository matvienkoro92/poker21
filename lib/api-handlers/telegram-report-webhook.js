...
async function sendRomanTotal(chatId, messageId, data = latestUnionData, requestInput = true) {
  const [mainBalance, sentValues] = await Promise.all([
...
async function sendCommands(chatId, messageId) {
  ...
  const lines = [
    ...
    "<b>/отмена</b> — отменить ожидающий ввод после /итого Роман",
...
module.exports = async function handler(req, res) {
  ...
  if (isCancelCommand(message.text)) {
    ...
    const result = await redisPipeline(
      ...
      { context: "telegram-report.roman-total.pending.cancel", timeoutMs: 2000 },
    );
    const cancelled = Number(result?.[0]?.result || 0) > 0;
    const sent = await telegram("sendMessage", {
      chat_id: chatId,
      text: cancelled ? "✅ Ввод после /итого Роман отменён." : "Сейчас нет активного ввода для отмены.",
