...
function parsePaymentDetailsMessage(text, awaitingInput = false) {
  ...
  return command?.action === "publish" || command?.reason === "amount_limit" || awaitingInput ? command : null;
}

function paymentDetailsFormText(error = false, reason = "") {
  return [
    reason === "amount_limit" ? "Максимальная сумма одной заявки — 10 000 ₽. Уменьшите сумму и отправьте данные ещё раз." : error
    ...
    "",
    "Сумма",
    "Максимум для заявки в рублях: 10 000 ₽ включительно.",
    "Номер телефона",
    "Банк",
    "Имя получателя",
    "",
...
async function sendBoundUnionPlayerProfile(chatId, binding, query, data = latestUnionData, footer = "", editMessageId = null) {
  ...
  const matches = exact ? [exact] : players
    ...
    .map(({ row }) => row);
  if (!matches.length) {
    const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", { ...(editMessageId ? { message_id: editMessageId } : {}), chat_id: chatId, text: `Игрок «${query}» в союзе «${binding.league}» не найден.${footer}` });
    return Boolean(sent.ok);
  }
  if (matches.length > 1 && lookupScore(matches[0].nick, query) === lookupScore(matches[1].nick, query)) {
    const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", { ...(editMessageId ? { message_id: editMessageId } : {}),
      chat_id: chatId,
  ...
  const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", { ...(editMessageId ? { message_id: editMessageId } : {}), chat_id: chatId, text: lines.join("\n"), parse_mode: "HTML" });
  return Boolean(sent.ok);
}
...
async function sendBoundClubPlayerProfile(chatId, binding, query, data = latestUnionData, footer = "", editMessageId = null) {
  const club = boundClubData(binding, data);
  const players = Array.isArray(club?.playerRows) ? club.playerRows : [];
  ...
  const matches = exact ? [exact] : players
    ...
    .map(({ row }) => row);
  if (!matches.length) {
    const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", { ...(editMessageId ? { message_id: editMessageId } : {}), chat_id: chatId, text: `Игрок «${query}» в клубе «${binding.club}» не найден.${footer}` });
    return Boolean(sent.ok);
  }
  if (matches.length > 1 && lookupScore(matches[0].nick, query) === lookupScore(matches[1].nick, query)) {
    const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", { ...(editMessageId ? { message_id: editMessageId } : {}),
...
module.exports = async function handler(req, res) {
  ...
  if (await acceptsPlayerSearchMessage(message)) {
    ...
    const footer = "\n\nЕсли хотите продолжить поиск, то просто продолжайте вводить ник либо введите /завершить, чтобы выйти из поиска.";
    const sent = binding.type === "union"
      ? await sendBoundUnionPlayerProfile(chatId, binding, query, latestUnionData, footer, message.playerSearchMessageId)
      : await sendBoundClubPlayerProfile(chatId, binding, query, latestUnionData, footer, message.playerSearchMessageId);
