...
function lookupScore(candidate, query) {
  const value = normalizeLookup(candidate);
  ...
  return distance <= maxDistance ? 3 + distance / 10 : null;
}

function findClubMatches(query) {
...
async function sendClubProfile(chatId, messageId, query) {
  const matches = findClubMatches(query);
  if (!matches.length) {
  ...
  if (matches.length > 1 && lookupScore(matches[0].name, query) !== 0) {
...
async function sendPlayerProfile(chatId, messageId, query) {
  const matches = findPlayerMatches(query);
  if (!matches.length) {
  ...
  if (matches.length > 1 && topScore !== 0 && topScore === secondScore && String(matches[0].id) !== String(query).trim()) {
    const lines = [
      ...
      "",
      ...matches.map((player) => `${escapeTelegramHtml(player.nick)} (${player.id}) — ${player.clubs.map(clubNameById).map(escapeTelegramHtml).join(", ")} — <code>/игрок ${player.id}</code>`),
    ];
    const sent = await telegram("sendMessage", { chat_id: chatId, text: lines.join("\n"), parse_mode: "HTML", reply_to_message_id: messageId });
    return Boolean(sent.ok);
  }
  ...
  const lines = [
    ...
    `<b>Период: ${displayIso(unionDirectory.startDate)}–${displayIso(unionDirectory.endDate)}</b>`,
    "",
    `Клубы: ${player.clubs.map(clubNameById).map(escapeTelegramHtml).join(", ") || "—"}`,
...
async function sendPlayerTops(chatId, messageId, type) {
  const configs = {
...
async function sendJackpot(chatId, messageId) {
  ...
  const totalFee = regularFee + jackpot21Fee;
  const totalPayout = regularPayout + jackpot21Payout;
  const net = totalFee - totalPayout;
  const lines = [
    "Джекпот союза",
    `<b>Период: ${displayIso(unionJackpotSummary.startDate)}–${displayIso(unionJackpotSummary.endDate)}</b>`,
    "",
    `Сбор обычного джекпота — ${formatRake(regularFee)}`,
    `Выплаты обычного джекпота — ${formatRake(regularPayout)}`,
    `Сбор Jackpot 21 — ${formatRake(jackpot21Fee)}`,
    `Выплаты Jackpot 21 — ${formatRake(jackpot21Payout)}`,
    "",
    `<b>Всего собрано: ${formatRake(totalFee)}</b>`,
    `<b>Всего выплачено: ${formatRake(totalPayout)}</b>`,
    `<b>Сборы минус выплаты: ${formatRake(net)}</b>`,
  ];
  const sent = await telegram("sendMessage", {
    chat_id: chatId,
    text: lines.join("\n"),
...
async function sendGames(chatId, messageId) {
...
async function sendOverlays(chatId, messageId) {
...
async function sendRakeSummary(chatId, messageId) {
...
module.exports = async function handler(req, res) {
  ...
  const clubQuery = parseEntityCommand(message.text, "клуб");
  if (clubQuery) {
    const sent = await sendClubProfile(chatId, message.message_id, clubQuery);
    return res.status(200).json({ ok: true, club: true, sent });
  }

  const playerQuery = parseEntityCommand(message.text, "игрок");
  if (playerQuery) {
    const sent = await sendPlayerProfile(chatId, message.message_id, playerQuery);
    return res.status(200).json({ ok: true, player: true, sent });
  }

  const playersCommand = parsePlayersCommand(message.text);
  if (playersCommand) {
    const sent = await sendPlayerTops(chatId, message.message_id, playersCommand);
    return res.status(200).json({ ok: true, players: playersCommand, sent });
  }

  if (isJackpotCommand(message.text)) {
    const sent = await sendJackpot(chatId, message.message_id);
    return res.status(200).json({ ok: true, jackpot: true, sent });
  }

  if (isGamesCommand(message.text)) {
    const sent = await sendGames(chatId, message.message_id);
    return res.status(200).json({ ok: true, games: true, sent });
  }

  if (isOverlaysCommand(message.text)) {
    const sent = await sendOverlays(chatId, message.message_id);
  ...
  if (isSummaryCommand(message.text)) {
    const sent = await sendRakeSummary(chatId, message.message_id);
