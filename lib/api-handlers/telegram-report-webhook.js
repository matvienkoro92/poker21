...
async function sendCommands(chatId, messageId) {
  ...
  const lines = [
    ...
    ...(!antiregMain ? [
      ...
      ...(isMainReportChat(chatId) ? ["<b>/баланс реквизиты</b> — балансы клубов и союзов по подтверждённым оплатам реквизитов"] : []),
...
async function getPaymentBalanceHistory(chatId) {
  if (!isRedisConfigured()) return [];
  ...
  for (let offset = 0; offset < keys.length; offset += 100) {
    ...
    for (const result of results) {
      ...
      history.push({
        ...
        comment: fee ? `Включён вычет 1%: ${formatRake(fee / 100)} ${item.currency === "usd" ? "$" : "₽"}` : "",
      });
    }
  }
  return history.sort((a, b) => String(b.timestamp || "").localeCompare(String(a.timestamp || ""))).slice(0, 5);
}
...
async function sendChatBalance(chatId, binding, editMessageId = null) {
  const balance = await getChatBalance(chatId, 5);
  ...
  if (balance.history.length) lines.push("", "Последние 5 операций:", "", formatBalanceHistoryBlocks(balance.history));
  ...
  if (paymentHistory.length) lines.push("", "Последние 5 операций по реквизитам:", "", formatBalanceHistoryBlocks(paymentHistory));
...
async function sendChatBalanceHistory(chatId, binding, editMessageId = null) {
  ...
  if (balance.history.length) lines.push("", formatBalanceHistoryBlocks(balance.history));
