...
function parseBalanceCommand(text) {
  ...
  for (const part of parts) {
    const valueMatch = part.match(/^([=]?\s*[+-]?\s*[\d\s\u00a0\u202f]+(?:\.\d{1,2})?)\s*(р|руб(?:лей|ля|ль)?|₽|\$|usd|дол(?:лар(?:ов|а)?)?)?$/iu);
    if (!valueMatch) return { action: "invalid" };
    const raw = valueMatch[1].replace(/[\s\u00a0\u202f]/g, "");
    const explicitSet = raw.startsWith("=");
    const numeric = Number(explicitSet ? raw.slice(1) : raw);
    ...
    const currency = String(valueMatch[2] || "р").toLowerCase();
...
async function sendChatBalance(chatId) {
  ...
  const lines = balance.cents == null && balance.usdCents == null
    ...
    : ["<b>Текущий баланс:</b>", ...(balance.cents == null ? [] : [`${formatRake(balance.cents / 100)} ₽`]), ...(balance.usdCents == null ? [] : [`${formatRake(balance.usdCents / 100)} $`])];
    ...
    lines.push("", "Последние изменения:", ...balance.history.map((entry) => {
      ...
      const append = (change, symbol) => {
        ...
        changes.push(change.action === "adjust"
          ? `${Number(change.cents || 0) >= 0 ? "+" : ""}${formatRake(Number(change.cents || 0) / 100)} ${symbol}`
          : `установлен ${formatRake(Number(change.cents || 0) / 100)} ${symbol}`);
      };
      append(entry.rub, "₽");
      append(entry.usd, "$");
      if (!changes.length && entry.action) append({ action: entry.action, cents: entry.action === "adjust" ? entry.delta : entry.cents }, "₽");
      return `${changes.join(", ")} — ${escapeTelegramHtml(entry.actor || "администратор")}`;
    }));
  }
  const sent = await telegram("sendMessage", { chat_id: chatId, text: lines.join("\n"), parse_mode: "HTML" });
  return Boolean(sent.ok);
  ...
  if (clubBinding) {
    ...
    if (balanceCommand) {
      ...
      if (balanceCommand.action === "invalid") {
        const sent = await telegram("sendMessage", { chat_id: chatId, text: "Формат: /баланс, /баланс 300р, 40$, /баланс +50$ или /баланс -100р" });
      ...
      const balances = changed ? [
        changed.cents == null ? null : `${formatRake(changed.cents / 100)} ₽`,
        changed.usdCents == null ? null : `${formatRake(changed.usdCents / 100)} $`,
      ].filter(Boolean).join("\n") : "";
      const sent = await telegram("sendMessage", {
        chat_id: chatId,
        text: changed
          ? `Изменение: ${operations}\n\n<b>Текущий баланс:</b>\n${balances}`
