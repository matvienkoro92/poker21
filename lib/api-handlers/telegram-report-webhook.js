...
function parseClubBindingCommand(text) {
  ...
  return match ? { type: match[1]?.toLowerCase() === "союз" ? "union" : "club", query: match[2].trim() } : null;
}
...
async function sendCurrentBoundClub(chatId, binding, data = latestUnionData) {
  if (binding.type === "union") {
    ...
    if (report) {
      ...
      const sent = await telegram("sendPhoto", { chat_id: chatId, photo: `${APP_ORIGIN}${report.imagePath}?v=bound-union-1`, caption: lines.join("\n"), parse_mode: "HTML" });
      return Boolean(sent.ok);
    }
...
module.exports = async function handler(req, res) {
  ...
  if (bindingRequest) {
    ...
    const isUnion = bindingRequest.type === "union";
    const candidates = isUnion
      ? (latestUnionData.jackpot?.leagues || []).map((row) => ({ id: String(row.leagueId), name: row.league }))
      : (latestUnionData.directory?.clubs || []);
    const exact = candidates.find((row) => String(row.id) === bindingRequest.query || normalizeLookup(row.name) === normalizeLookup(bindingRequest.query));
    const matches = exact ? [exact] : isUnion
      ? candidates.map((row) => ({ ...row, score: lookupScore(row.name, bindingRequest.query) })).filter((row) => row.score !== null).sort((a, b) => a.score - b.score).slice(0, 10)
      : findClubMatches(bindingRequest.query, latestUnionData);
    const entity = matches.length === 1 ? matches[0] : null;
    if (!entity && matches.length > 1) {
      const sent = await telegram("sendMessage", {
        chat_id: chatId,
        text: [
          `Найдено несколько ${isUnion ? "союзов" : "клубов"} по запросу «${escapeTelegramHtml(bindingRequest.query)}»:`,
          "",
          ...matches.map((row) => `${escapeTelegramHtml(row.name)} — <code>${row.id}</code>`),
          "",
          `Повторите команду с точным ID, например: <code>/привязать ${isUnion ? "союз 184691" : "клуб 758417"}</code>`,
        ].join("\n"),
        parse_mode: "HTML",
      });
    ...
    if (!entity) {
      const sent = await telegram("sendMessage", { chat_id: chatId, text: `${isUnion ? "Союз" : "Клуб"} «${bindingRequest.query}» не найден в актуальном отчёте.` });
  ...
  if (isBindingStatusCommand(message.text)) {
    ...
    const text = clubBinding
      ? `Группа привязана к ${clubBinding.type === "union" ? "союзу" : "клубу"} «${boundName}» (${boundId}). Хранилище: ${isRedisConfigured() ? "Redis" : "временная локальная память"}.`
      : isRedisConfigured()
