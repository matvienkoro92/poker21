...
function parseBalanceCommand(text) {
  const match = String(text || "").trim().match(/^\/баланс(?:@[A-Za-z0-9_]+)?(?:\s+([=]?\s*[+-]?\s*[\d\s\u00a0\u202f]+(?:[.,]\d{1,2})?))?\s*$/iu);
  ...
  if (!explicitSet && (raw.startsWith("+") || raw.startsWith("-"))) return { action: "adjust", cents };
  return { action: "set", cents };
}
...
async function getChatBalance(chatId) {
  if (isRedisConfigured()) {
    ...
    const history = Array.isArray(result?.[1]?.result)
      ? result[1].result.map((item) => { try { return JSON.parse(String(item)); } catch (_) { return null; } }).filter(Boolean)
      : [];
    return { cents: raw == null ? null : Number(raw), history };
  }
  return chatBalances.get(String(chatId)) || { cents: null, history: [] };
}
...
async function changeChatBalance(chatId, command, user) {
  ...
  if (isRedisConfigured()) {
    ...
    if (command.action === "adjust") {
      const result = await redisPipeline([["INCRBY", chatBalanceKey(chatId), String(command.cents)]], { context: "telegram-report.chat-balance.adjust", timeoutMs: 2000 });
      cents = Number(result?.[0]?.result);
    } else {
      const result = await redisPipeline([["SET", chatBalanceKey(chatId), String(command.cents)]], { context: "telegram-report.chat-balance.set", timeoutMs: 2000 });
      if (result?.[0]?.result !== "OK") return null;
      cents = command.cents;
    }
    if (!Number.isFinite(cents)) return null;
    const entry = JSON.stringify({ action: command.action, delta: command.action === "adjust" ? command.cents : null, cents, actor, timestamp });
    await redisPipeline([
  ...
  const current = chatBalances.get(String(chatId)) || { cents: 0, history: [] };
  const cents = command.action === "adjust" ? Number(current.cents || 0) + command.cents : command.cents;
  const entry = { action: command.action, delta: command.action === "adjust" ? command.cents : null, cents, actor, timestamp };
  chatBalances.set(String(chatId), { cents, history: [entry, ...current.history].slice(0, 20) });
  return { cents, actor };
}
...
async function sendClubReports(chatId, messageId, data = latestUnionData) {
  const { reports, groups } = getClubReportGroups(data);
  let allSent = reports.length > 0;
  for (const [groupIndex, group] of groups.entries()) {
    if (group.reports.length === 0) continue;
...
async function sendChatBalance(chatId) {
  ...
  const lines = [
    balance.cents == null ? "<b>Текущий баланс ещё не задан.</b>" : `<b>Текущий баланс: ${formatRake(balance.cents / 100)}</b>`,
  ];
  if (balance.history.length) {
    lines.push("", "Последние изменения:", ...balance.history.map((entry) => {
      const operation = entry.action === "adjust"
        ? `${Number(entry.delta || 0) >= 0 ? "+" : ""}${formatRake(Number(entry.delta || 0) / 100)}`
        : `установлен ${formatRake(Number(entry.cents || 0) / 100)}`;
      return `${operation} — ${escapeTelegramHtml(entry.actor || "администратор")}`;
    }));
  }
  const sent = await telegram("sendMessage", { chat_id: chatId, text: lines.join("\n"), parse_mode: "HTML" });
  return Boolean(sent.ok);
...
async function sendBoundClubCommands(chatId, binding) {
  if (binding.type === "union") {
    const lines = [
      ...
      "<b>/баланс 10000</b> — установить баланс (администратор)",
      "<b>/баланс +500</b> или <b>/баланс -300</b> — скорректировать баланс",
      "",
      "<b>Клубы</b>",
...
module.exports = async function handler(req, res) {
  ...
  const callbackText = callbackQuery ? callbackCommands[String(callbackQuery.data || "")] : null;
  ...
  if (clubBinding) {
    ...
    if (balanceCommand) {
      ...
      if (balanceCommand.action === "invalid") {
        const sent = await telegram("sendMessage", { chat_id: chatId, text: "Формат: /баланс, /баланс 10000, /баланс +500 или /баланс -300" });
      ...
      const operation = balanceCommand.action === "adjust"
        ? `Корректировка: ${balanceCommand.cents >= 0 ? "+" : ""}${formatRake(balanceCommand.cents / 100)}\n`
        : "";
      const sent = await telegram("sendMessage", {
        chat_id: chatId,
        text: changed
          ? `${operation}<b>Текущий баланс: ${formatRake(changed.cents / 100)}</b>`
    ...
    if (clubBinding.type === "union" && boundPlayersCommand) {
      const sent = await sendBoundUnionPlayerTops(chatId, clubBinding, boundPlayersCommand);
      return res.status(200).json({ ok: true, clubMode: true, unionPlayers: boundPlayersCommand, sent });
    }
    const boundPlayerQuery = clubBinding.type === "union" ? parseEntityCommand(message.text, "игрок") : null;
    if (boundPlayerQuery) {
      const sent = await sendBoundUnionPlayerProfile(chatId, clubBinding, boundPlayerQuery);
      return res.status(200).json({ ok: true, clubMode: true, unionPlayer: boundPlayerQuery, sent });
    }
  ...
  if (isClubsCommand(unionText)) {
    ...
    const sent = await sendClubReports(chatId, message.message_id, unionData);
