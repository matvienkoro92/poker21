...
async function getPaymentDetails(id) {
  ...
  try { return JSON.parse(String(result?.[0]?.result || "")); } catch (_) { return null; }
}
...
async function listPaymentDetails() {
  ...
  return result.map((row) => { try { return JSON.parse(String(row?.result || "")); } catch (_) { return null; } }).filter(Boolean);
}
...
async function sendPaymentDetailsRegistry(chatId, allowActions = true) {
  ...
  const active = (await listPaymentDetails()).filter((item) => ["open", "claimed", "paid"].includes(item.status));
  if (!active.length) {
    const sent = await telegram("sendMessage", {
      chat_id: chatId,
      text: [
  ...
  for (const item of active) {
    const status = item.status === "open" ? "Ожидает плательщика" : item.status === "claimed" ? `В работе у «${item.payer?.name || "—"}»` : "Оплачено, ожидает подтверждения";
    const buttons = allowActions && item.status === "open"
      ? [[{ text: "Взять в работу", callback_data: `payreq:take:${item.id}` }]]
      : allowActions && item.status === "claimed"
...
module.exports = async function handler(req, res) {
  ...
  const paymentCallback = callbackQuery ? String(callbackQuery.data || "").match(/^payreq:(take|paid|confirm|reject):([A-Za-z0-9_-]+)$/) : null;
  if (paymentCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    if (action === "take") {
      ...
      const claim = await redisPipeline([["SET", paymentDetailsClaimKey(id), actorChatId, "NX", "EX", "86400"]], { context: "telegram-report.payment-details.claim", timeoutMs: 2000 });
    ...
    if (action === "reject") {
      ...
      await notifyPaymentParty(item.payer.chatId, `<b>${escapeTelegramHtml(item.owner.name)}</b> сообщил, что платёж <b>${formatPaymentAmount(item)}</b> пока не поступил. Проверьте перевод.`, [[{ text: "Я оплатил", callback_data: `payreq:paid:${id}` }]]);
      return res.status(200).json({ ok: true, paymentDetails: true, action });
    }
