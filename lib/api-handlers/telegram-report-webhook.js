...
function isScheduleCommand(text) {
  return /^\/(?:расписание|schedule)(?:@[A-Za-z0-9_]+)?\s*$/iu.test(String(text || "").trim());
}
...
function scheduleIsToday(row, dateParts) {
  ...
  if (scheduleCategory(text) === "daily" || /турнир(?:ы|ов)?\s+дня|сегодня/iu.test(text)) return true;
...
async function sendTournamentSchedule(chatId, messageId) {
  ...
  const todayMain = todayRows.filter((row) => ["month", "day"].includes(scheduleCategory(row.text)));
  const todayRegular = todayRows.filter((row) => scheduleCategory(row.text) === "daily");
  const categories = [
  ...
  const lines = [
    ...
    "",
    ...(todayMain.length
      ? ["<b>ГЛАВНЫЕ ТУРНИРЫ</b>", "", ...todayMain.map((row) => scheduleEntryText(row, rows.indexOf(row), true))]
      : []),
    ...(todayRegular.length
      ? [todayMain.length ? "" : "<b>СЕГОДНЯШНИЕ ТУРНИРЫ</b>", "", ...todayRegular.map((row) => scheduleEntryText(row, rows.indexOf(row)))]
      : []),
  ...
  for (const category of categories) {
    const categoryRows = rows.filter((row) => scheduleCategory(row.text) === category.key);
    if (!categoryRows.length) continue;
    lines.push("", category.title, "", ...categoryRows.map((row) => scheduleEntryText(row, rows.indexOf(row), category.key === "month")));
  }
  if (!rows.length) lines.push("", "Расписание пока не заполнено.");
...
async function sendCommands(chatId, messageId) {
  ...
  const lines = [
    ...
    "<b>Расписание</b>",
    "<b>/расписание</b> — показать расписание турниров",
...
module.exports = async function handler(req, res) {
  ...
  const scheduleCallback = callbackQuery ? String(callbackQuery.data || "").match(/^schedule:(add|edit|delete)(?::([A-Za-z0-9_-]+))?$/) : null;
  if (scheduleCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    const itemId = scheduleCallback[2] || "";
    ...
    const rows = await getTournamentSchedule(actorChatId);
    if ((action === "edit" || action === "delete") && !itemId) {
      if (!rows.length) {
      ...
      const buttons = rows.slice(0, 50).map((row, index) => [{
        text: `${index + 1}. ${String(row.text).replace(/\s+/g, " ").slice(0, 45)}`,
        callback_data: `schedule:${action}:${row.id}`,
      }]);
      await telegram("sendMessage", {
        chat_id: actorChatId,
        text: action === "edit" ? "Какой турнир изменить?" : "Какой турнир удалить?",
        reply_markup: { inline_keyboard: buttons },
      });
      return res.status(200).json({ ok: true, schedule: true, action, selection: true });
    }
    ...
    if (pending?.[0]?.result) {
      ...
      if (!state || !["add", "edit"].includes(state.action)) {
        ...
        if (state.action === "edit") {
          ...
          savedRows = [...rows, { id, text: tournamentText, createdAt: new Date().toISOString() }];
        }
        ...
        const sent = await telegram("sendMessage", {
          ...
          text: state.action === "edit"
            ? "✅ Турнир изменён.\n\nПоказать список: /расписание"
            : "✅ Турнир добавлен. Отправьте следующую строку или нажмите /готово.",
          reply_to_message_id: message.message_id,
        });
