...
async function sendUnionReports(chatId, messageId, data = latestUnionData) {
  ...
  const groups = [
    { heading: "❗ ДЛЯ РОМАНА:", recipient: "Романа", reports: romanReports },
    { heading: "❗ ДЛЯ СЕРГЕЯ:", recipient: "Сергея", reports: sergeyReports },
    { heading: "❗ ДЛЯ ИЛЬИ:", recipient: "Ильи", reports: ilyaReports },
  ];
  const groupTotals = [];
  for (const [groupIndex, group] of groups.entries()) {
    if (group.reports.length === 0) continue;
    const groupTotal = group.reports.reduce(
      (sum, report) => sum + Number(report.metrics?.total || 0),
      0,
    );
    groupTotals.push({ recipient: group.recipient, total: groupTotal });
  ...
  const totalsSent = await telegram("sendMessage", {
    ...
    text: groupTotals
      .map(({ recipient, total }) => {
        return `<b>ИТОГО сумма по всем союзам ${recipient}: ${formatRake(total)}</b>`;
      })
      .join("\n"),
    parse_mode: "HTML",
  });
