...
async function mergeRequisiteBalances(operationId, actor) {
  ...
  let transfers = [];
  try { const parsed = JSON.parse(rows[0].result[2] || "[]"); if (Array.isArray(parsed)) transfers = parsed; } catch (_) {}
  const notifications = await Promise.all(transfers.map(transfer => telegram("sendMessage", {
    chat_id: String(transfer.chatId),
    parse_mode: "HTML",
    text: [
      "✅ <b>Баланс реквизитов перенесён в основной</b>",
      `Перенесено: <b>${formatBalanceAmount(transfer.amount, "₽", true)}</b>`,
      "",
      `<b>Клубный баланс:</b> ${formatBalanceAmount(transfer.before, "₽")} → ${formatBalanceAmount(transfer.after, "₽")}`,
      `<b>Баланс реквизитов:</b> ${formatBalanceAmount(transfer.amount, "₽")} → ${formatBalanceAmount(0, "₽")}`,
      "",
      "История сохранена. Дополнительная комиссия за перенос не списывалась.",
    ].join("\n"),
  })));
  await Promise.all(accounts.map(a => refreshMenu(a.chatId, telegram)));
  return { count: Number(rows[0].result[0]), cents: Number(rows[0].result[1]), notificationFailures: notifications.filter(row => !row?.ok).length };
}
...
module.exports = async function handler(req, res) {
  ...
  if (paymentCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    await telegram("sendMessage", {
      chat_id: "-1004472155269",
      text: completedRequisiteDealText(item, ownerAfter, payerAfter),
      parse_mode: "HTML",
    });
    await sendOrEditPaymentMessage(actorChatId, [
