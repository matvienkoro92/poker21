...
function parsePaymentDetailsCommand(text) {
  const match = String(text || "").trim().match(/^\/(?:реквизиты|реквизты|платежи)(?:@[A-Za-z0-9_]+)?(?:\s+([\s\S]+?))?\s*$/iu);
  if (!match) return null;
  if (!match[1]) return { action: "list" };
  const payload = match[1].trim();
  if (/^(?:удалить|закрыть)$/iu.test(payload)) return { action: "remove" };
  const parts = payload.includes("|")
    ? payload.split(/\s*\|\s*/u)
    : payload.split(/\r?\n/u).map((line) => line.trim()).filter(Boolean);
  if (parts.length < 2) return { action: "invalid" };
...
async function reopenExpiredPaymentDetails(item) {
  if (!item || item.status !== "claimed" || !item.claimedAt) return item;
...
async function getChatBalance(chatId, historyLimit = 3) {
  ...
  return { ...balance, history: balance.history.slice(0, Math.max(0, Number(historyLimit) || 0)) };
}
...
async function sendPaymentDetailsRegistry(chatId, allowActions = true) {
  if (!isRedisConfigured()) {
    const sent = await telegram("sendMessage", { chat_id: chatId, text: "Реестр реквизитов недоступен: Redis не настроен." });
    return Boolean(sent.ok);
  }
  const active = (await listPaymentDetails()).filter((item) => {
    if (!["open", "claimed", "paid"].includes(item.status)) return false;
  ...
  if (!active.length) {
    const sent = await telegram("sendMessage", {
      ...
      text: [
        ...
        "",
        "Для размещения напишите каждое значение с новой строки:",
        "/реквизиты",
        "5000р",
        "+7999-999-99-99",
        "Сбер",
  ...
  for (const item of active) {
    const secondsLeft = item.status === "claimed"
    ...
    const status = item.status === "open"
      ? "Ожидает плательщика"
      : item.status === "claimed"
        ? `В работе у «${item.payer?.name || "—"}», осталось до ${minutesLeft} мин.`
        : "Оплачено, ожидает подтверждения";
    const buttons = allowActions && item.status === "open"
      ? [[{ text: "Взять в работу", callback_data: `payreq:take:${item.id}` }]]
      : allowActions && item.status === "claimed" && String(item.payer?.chatId) === String(chatId)
        ? [[
          { text: "Я оплатил", callback_data: `payreq:paid:${item.id}` },
          { text: "Отменить", callback_data: `payreq:cancel:${item.id}` },
        ]]
        : [];
    await telegram("sendMessage", {
      chat_id: chatId,
      text: [
        `<b>${escapeTelegramHtml(item.owner.name)}</b> — <b>${formatPaymentAmount(item)}</b>`,
...
async function sendAllChatBalances(chatId, messageId) {
  ...
  const unionLines = unionRows.map((row) => {
  ...
  const clubLines = clubRows.map((row) => {
...
async function sendBoundClubCommands(chatId, binding) {
  if (binding.type === "union") {
    const lines = [
      ...
      "Размещение: <code>/реквизиты</code>, сумма, телефон/карта, банк и получатель — каждый с новой строки",
...
module.exports = async function handler(req, res) {
  ...
  const paymentCallback = callbackQuery ? String(callbackQuery.data || "").match(/^payreq:(take|paid|cancel|confirm|reject):([A-Za-z0-9_-]+)$/) : null;
  if (paymentCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    if (action === "take") {
      ...
      const claim = await redisPipeline([["SET", paymentDetailsClaimKey(id), actorChatId, "NX", "EX", "600"]], { context: "telegram-report.payment-details.claim", timeoutMs: 2000 });
    ...
    if (action === "cancel") {
      ...
      await notifyPaymentParty(actorChatId, `Оплата <b>${formatPaymentAmount(item)}</b> отменена. Заявка возвращена в общий список.`);
    ...
    if (action === "paid") {
      ...
      await savePaymentDetails(item);
      await notifyPaymentParty(actorChatId, `Оплата <b>${formatPaymentAmount(item)}</b> отмечена. Ожидаем подтверждения получателя.`);
      await notifyPaymentParty(item.owner.chatId, `<b>${escapeTelegramHtml(item.payer.name)}</b> сообщил об оплате <b>${formatPaymentAmount(item)}</b>.\n\nПроверьте поступление и подтвердите.`, [[
        { text: "Подтвердить получение", callback_data: `payreq:confirm:${id}` },
    ...
    if (action === "reject") {
      ...
      item.claimedAt = new Date().toISOString();
      await savePaymentDetails(item);
      await redisPipeline([["SET", paymentDetailsClaimKey(id), String(item.payer.chatId), "EX", "600"]], { context: "telegram-report.payment-details.retry", timeoutMs: 2000 });
      await notifyPaymentParty(actorChatId, "Отмечено: платёж пока не поступил.");
    ...
    await savePaymentDetails(item);
    await redisPipeline([["DEL", paymentDetailsClaimKey(id)]], { context: "telegram-report.payment-details.confirm", timeoutMs: 2000 });
    await notifyPaymentParty(actorChatId, `✅ Получение <b>${formatPaymentAmount(item)}</b> подтверждено. Заявка закрыта.`);
    await notifyPaymentParty(item.payer.chatId, `✅ <b>${escapeTelegramHtml(item.owner.name)}</b> подтвердил получение <b>${formatPaymentAmount(item)}</b>.`);
    return res.status(200).json({ ok: true, paymentDetails: true, action });
  }
  ...
  if (!clubBinding && isMainReportChat(chatId) && mainPaymentDetailsCommand) {
  ...
  if (clubBinding) {
    const paymentDetailsCommand = parsePaymentDetailsCommand(message.text);
    if (paymentDetailsCommand) {
      ...
      if (paymentDetailsCommand.action === "invalid") {
        const sent = await telegram("sendMessage", {
          chat_id: chatId,
          text: [
            "Формат размещения — каждое значение с новой строки:",
            "",
            "/реквизиты",
            "5000р",
            "+7999-999-99-99",
      ...
      const ownActive = currentItems.filter((item) => String(item.owner?.chatId) === chatId && ["open", "claimed", "paid"].includes(item.status));
      ...
      if (ownActive.some((item) => item.status !== "open")) {
        const sent = await telegram("sendMessage", { chat_id: chatId, text: "Сначала завершите текущую заявку: её уже взяли в работу." });
        return res.status(200).json({ ok: true, clubMode: true, paymentDetails: false, busy: true, sent: Boolean(sent.ok) });
      }
      await Promise.all(ownActive.map(async (item) => { item.status = "replaced"; await savePaymentDetails(item); }));
      ...
      if (saved) await redisPipeline([["LPUSH", PAYMENT_DETAILS_INDEX_KEY, id], ["LTRIM", PAYMENT_DETAILS_INDEX_KEY, "0", "199"]], { context: "telegram-report.payment-details.publish", timeoutMs: 2000 });
      const sent = await telegram("sendMessage", {
        chat_id: chatId,
        text: saved
      ...
      return res.status(200).json({ ok: true, clubMode: true, paymentDetails: saved, sent: Boolean(sent.ok) });
    }
    if (isPaymentConfirmCommand(message.text)) {
      ...
      for (const item of pending) {
        await notifyPaymentParty(chatId, `<b>${escapeTelegramHtml(item.payer?.name || "Плательщик")}</b> сообщил об оплате <b>${formatPaymentAmount(item)}</b>.`, [[
          { text: "Подтвердить получение", callback_data: `payreq:confirm:${item.id}` },
