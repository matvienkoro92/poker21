...
async function acceptsPlayerSearchMessage(message) {
  ...
  return explicitReply || Boolean(rows?.[0]?.result);
}
...
async function sendBoundUnionPlayerProfile(chatId, binding, query, data = latestUnionData, footer = "") {
  ...
  const matches = exact ? [exact] : players
    ...
    .map(({ row }) => row);
  if (!matches.length) {
    const sent = await telegram("sendMessage", { chat_id: chatId, text: `Игрок «${query}» в союзе «${binding.league}» не найден.${footer}` });
    return Boolean(sent.ok);
  }
...
async function sendBoundClubPlayerProfile(chatId, binding, query, data = latestUnionData, footer = "") {
  const club = boundClubData(binding, data);
  const players = Array.isArray(club?.playerRows) ? club.playerRows : [];
  ...
  const matches = exact ? [exact] : players
    ...
    .map(({ row }) => row);
  if (!matches.length) {
    const sent = await telegram("sendMessage", { chat_id: chatId, text: `Игрок «${query}» в клубе «${binding.club}» не найден.${footer}` });
    return Boolean(sent.ok);
  }
  if (matches.length > 1 && lookupScore(matches[0].nick, query) === lookupScore(matches[1].nick, query)) {
    const sent = await telegram("sendMessage", {
...
module.exports = async function handler(req, res) {
  ...
  if (pulsePlayerSearchCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    if (sent.ok && isRedisConfigured()) {
      await redisPipeline([["SET", playerSearchPendingKey(actorChatId, callbackQuery.from.id), "1", "EX", 600]], {
  ...
  if (await acceptsPlayerSearchMessage(message)) {
    ...
    const sent = binding.type === "union"
      ? await sendBoundUnionPlayerProfile(chatId, binding, query, latestUnionData, footer)
      : await sendBoundClubPlayerProfile(chatId, binding, query, latestUnionData, footer);
