...
async function sendBoundUnionClubs(chatId, binding, data = latestUnionData) {
  ...
  const activePlayers = leaguePlayers.length
    ? leaguePlayers.filter((row) => [row.rake, row.winnings, row.insurance, row.jackpotFee, row.jackpotPayout].some((value) => Number(value || 0) !== 0)).length
  ...
  const lines = [
    ...
    "",
    `<b>Игроков всего: ${formatInteger(totalPlayers)}</b>`,
    `<b>Активных игроков: ${formatInteger(activePlayers)}</b>`,
