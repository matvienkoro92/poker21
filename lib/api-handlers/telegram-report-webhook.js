...
async function sendBoundReportsMenu(chatId, binding, messageId = null, editMessageId = null) {
  ...
  const text = periods.length
    ? `<b>Отчёты ${escapeTelegramHtml(title)}</b>\nВыберите период:`
    : `<b>Отчёты ${escapeTelegramHtml(title)}</b>\nДоступных отчётов пока нет.`;
  const replyMarkup = periods.length ? {
    inline_keyboard: periods.map((period, index) => [{
      text: `${index === 0 ? "Текущий · " : index === 1 ? "Прошлая неделя · " : ""}${displayIso(period.startDate, false)}–${displayIso(period.endDate, false)}`,
...
module.exports = async function handler(req, res) {
  ...
  if (reportsPeriodCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    if (!selectedPeriod) {
      const sent = await sendBoundReportsMenu(actorChatId, binding, null, callbackQuery.message.message_id);
