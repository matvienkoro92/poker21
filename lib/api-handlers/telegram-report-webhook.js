...
function pulseUnionClubsKeyboard(binding, selectedMetric = "", selectedMask = 1n) {
  ...
  return { inline_keyboard: [
    ...
    [{ text: "🔎 Клубы отдельно", callback_data: "pulse:unionclubs:info" }],
    [{ text: "⬅️ Назад", callback_data: "pulse:menu", style: "primary" }],
  ] };
}
  ...
  if ((pulseMenuCallback || pulsePlayerCallback || pulsePlayerPeriodCallback || pulsePlayerSelectCallback || pulsePlayerToggleCallback || pulseTotalCallback || pulseWeeksCallback || pulseWeeksToggleCallback || pulseWeeksApplyCallback || pulseUnionClubsCallback || pulseUnionClubsSelectCallback || pulseUnionClubsToggleCallback || pulseUnionClubTopCallback || pulseUnionClubInfoCallback || pulseUnionClubInfoToggleCallback || pulseScheduleViewCallback) && callbackQuery?.message?.chat?.id != null) {
    ...
    if (pulseMenuCallback) {
      ...
      if (section === "totals") {
        ...
        if (binding.type === "club") {
          if (!hasPhoto) await telegram("deleteMessage", { chat_id: actorChatId, message_id: callbackQuery.message.message_id });
          const sent = await sendBoundReportsMenu(
            actorChatId,
            binding,
            null,
            hasPhoto ? callbackQuery.message.message_id : null,
          );
          return res.status(200).json({ ok: true, pulseMenu: section, directClubReport: true, sent });
        }
        if (hasPhoto) {
