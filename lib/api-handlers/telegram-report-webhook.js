...
function pulseTotalsKeyboard(binding, reportShown = false) {
  return { inline_keyboard: [
    ...
    [{ text: "⬅️ Назад", callback_data: reportShown ? "pulse:total:choose" : "pulse:menu", style: "primary" }],
  ] };
}
...
async function sendTotal(chatId, messageId, command, servicePercent, clubId = null, editMessageId = null, binding = null) {
  ...
  if (binding) {
    ...
    try {
      ...
      const uploaded = await sendTelegramPhotoBuffer(
        chatId,
        image,
        fullCaption.length <= 1000 ? fullCaption : compactCaption,
        pulseTotalsKeyboard(binding, true),
        editMessageId,
      );
      ...
      const fallback = await telegram("sendMessage", {
        ...
        text: lines.join("\n"),
        parse_mode: "HTML",
        reply_markup: pulseTotalsKeyboard(binding, true),
      });
  ...
  const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", {
    chat_id: chatId,
    ...(editMessageId ? { message_id: editMessageId, reply_markup: pulseTotalsKeyboard(binding || { type: "club" }, Boolean(binding)) } : { reply_to_message_id: messageId }),
    text: lines.join("\n"),
    parse_mode: "HTML",
  });
  return Boolean(sent.ok);
...
module.exports = async function handler(req, res) {
  ...
  if ((pulseMenuCallback || pulsePlayerCallback || pulsePlayerPeriodCallback || pulsePlayerSelectCallback || pulsePlayerToggleCallback || pulseTotalCallback || pulseWeeksCallback || pulseWeeksToggleCallback || pulseWeeksApplyCallback || pulseUnionClubsCallback || pulseUnionClubsPeriodCallback || pulseUnionClubTopCallback || pulseUnionClubInfoCallback || pulseScheduleViewCallback) && callbackQuery?.message?.chat?.id != null) {
    ...
    if (pulseTotalCallback?.[1] === "choose") {
      const hasPhoto = Boolean(callbackQuery.message.photo?.length);
      if (hasPhoto) await telegram("deleteMessage", { chat_id: actorChatId, message_id: callbackQuery.message.message_id });
      const sent = await sendPulseWeeksMenu(actorChatId, binding, hasPhoto ? null : callbackQuery.message.message_id, 0n, false);
    ...
    if (pulseWeeksToggleCallback) {
      ...
      const sent = await sendPulseWeeksMenu(actorChatId, binding, callbackQuery.message.message_id, mask, Boolean(callbackQuery.message.photo?.length));
    ...
    if (pulseWeeksApplyCallback) {
      ...
      const hasPhoto = Boolean(callbackQuery.message.photo?.length);
      const sent = await sendTotal(actorChatId, null, { type: "selected", endDates }, DEFAULT_SERVICE_PERCENT, binding.type === "club" ? binding.clubId : null, hasPhoto ? callbackQuery.message.message_id : null, binding);
    ...
    const command = totalType === "weeks" ? { type: "weeks", count: selectedWeeks }
      : totalType === "month" ? { type: "previousMonth" }
        : { type: "all" };
    const hasPhoto = Boolean(callbackQuery.message.photo?.length);
    const sent = await sendTotal(actorChatId, null, command, DEFAULT_SERVICE_PERCENT, binding.type === "club" ? binding.clubId : null, hasPhoto ? callbackQuery.message.message_id : null, binding);
    if (sent && !hasPhoto) await telegram("deleteMessage", { chat_id: actorChatId, message_id: callbackQuery.message.message_id });
