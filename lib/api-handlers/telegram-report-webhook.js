...
async function sendBoundUnionPlayerTops(chatId, binding, type, data = latestUnionData, editMessageId = null) {
  ...
  const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", {
    chat_id: chatId,
    ...(editMessageId ? { message_id: editMessageId, reply_markup: pulsePlayersKeyboard() } : {}),
    text: lines.join("\n"),
    parse_mode: "HTML",
  });
  return Boolean(sent.ok);
...
async function sendInsightPlayers(chatId, binding, kind, editMessageId) {
  const configs = {
...
function insightPulseMetrics(period, binding) {
  ...
  const club = (period.directory?.clubs || []).find((row) => String(row.id) === String(binding.clubId));
  return { activePlayers: activeRows.length, rake: Number(club?.rake || 0) };
}
...
function pulseKeyboard(mode) {
  return { inline_keyboard: [
    [
      { text: `${mode === "week" ? "✓ " : ""}Неделя`, callback_data: "pulse:week" },
      { text: `${mode === "month" ? "✓ " : ""}Месяц`, callback_data: "pulse:month" },
    ],
    [{ text: "⬅️ Пульс", callback_data: "pulse:menu" }],
  ] };
}

function pulseMainKeyboard() {
  return { inline_keyboard: [
...
function pulsePlayersKeyboard() {
  return { inline_keyboard: [
    [
      { text: "💰 Рейк", callback_data: "pulse:player:rake" },
      { text: "📈 Плюс", callback_data: "pulse:player:plus" },
      { text: "📉 Минус", callback_data: "pulse:player:minus" },
    ],
    ...insightMenuKeyboard().inline_keyboard,
    [{ text: "⬅️ Пульс", callback_data: "pulse:menu" }],
  ] };
}
...
function pulseTotalsKeyboard(binding) {
  return { inline_keyboard: [
    ...
    ] : [[{ text: "Текущая неделя", callback_data: "pulse:total:union" }]]),
    [{ text: "⬅️ Пульс", callback_data: "pulse:menu" }],
  ] };
}
...
function availableConsecutiveClubReports(binding) {
  if (binding.type !== "club") return [];
  const reports = historicalClubReports(binding.clubId).sort((a, b) => String(b.endDate).localeCompare(String(a.endDate)));
...
async function sendPulseSubmenu(chatId, binding, type, messageId) {
  ...
  const sent = await telegram("editMessageText", {
    ...
    parse_mode: "HTML",
    reply_markup: isPlayers ? pulsePlayersKeyboard() : pulseTotalsKeyboard(binding),
  });
  return Boolean(sent.ok);
}
...
async function sendPulseMainMenu(chatId, binding, messageId = null, editMessageId = null) {
  ...
  const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", {
    ...
    parse_mode: "HTML",
    reply_markup: pulseMainKeyboard(),
  });
  return Boolean(sent.ok);
}
...
async function sendBoundClubPlayerTops(chatId, binding, type, data = latestUnionData, editMessageId = null) {
...
async function sendTotal(chatId, messageId, command, servicePercent, clubId = null, editMessageId = null, binding = null) {
  ...
  const historicalReports = clubId ? historicalClubReports(clubId) : [];
...
async function sendBoundClubCommands(chatId, binding) {
  if (binding.type === "union") {
    const lines = [
      ...
      "",
      "<b>Статистика</b>",
      "<b>/пульс</b> — динамика, игроки и итоги союза",
...
module.exports = async function handler(req, res) {
  ...
  const pulseMenuCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:(menu|dynamics|players|totals)$/) : null;
  const pulsePlayerCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:player:(rake|plus|minus)$/) : null;
  ...
  if ((pulseMenuCallback || pulsePlayerCallback || pulseTotalCallback || pulseWeeksCallback || pulseWeeksToggleCallback || pulseWeeksApplyCallback) && callbackQuery?.message?.chat?.id != null) {
    const actorChatId = String(callbackQuery.message.chat.id);
    await telegram("answerCallbackQuery", { callback_query_id: callbackQuery.id });
    const binding = await getClubBinding(actorChatId);
    if (!binding) {
    ...
    if (pulseMenuCallback) {
      ...
      if (section === "dynamics") {
        const sent = await sendBindingPulse(actorChatId, binding, null, "week", callbackQuery.message.message_id);
        return res.status(200).json({ ok: true, pulseMenu: section, sent });
      }
    ...
    if (pulsePlayerCallback) {
      ...
      const sent = binding.type === "union"
        ? await sendBoundUnionPlayerTops(actorChatId, binding, type, latestUnionData, callbackQuery.message.message_id)
        : await sendBoundClubPlayerTops(actorChatId, binding, type, latestUnionData, callbackQuery.message.message_id);
    ...
    if (pulseWeeksApplyCallback) {
      ...
      const sent = await sendTotal(actorChatId, null, { type: "selected", endDates }, DEFAULT_SERVICE_PERCENT, binding.clubId, callbackQuery.message.message_id, binding);
    ...
    const command = totalType === "weeks" ? { type: "weeks", count: selectedWeeks }
      : totalType === "month" ? { type: "previousMonth" }
        : { type: "all" };
    const sent = await sendTotal(actorChatId, null, command, DEFAULT_SERVICE_PERCENT, binding.clubId, callbackQuery.message.message_id, binding);
