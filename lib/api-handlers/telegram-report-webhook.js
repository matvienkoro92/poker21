...
function paymentDetailsFormText(error = false) {
  return [
    error ? "Не удалось распознать данные. Введите их ещё раз в формате:" : "Введите реквизиты в формате:",
    "",
    "5000р",
    "+7 999 999-99-99",
    "Сбер",
  ...
  if (clubBinding) {
    ...
    if (paymentDetailsCommand) {
      ...
      if (paymentDetailsCommand.action === "prompt") {
        ...
        const sent = await telegram("sendMessage", { chat_id: chatId, text: paymentDetailsFormText() });
      ...
      if (paymentDetailsCommand.action === "invalid") {
        ...
        const sent = await telegram("sendMessage", { chat_id: chatId, text: paymentDetailsFormText(true) });
