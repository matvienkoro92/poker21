...
function pulseMainKeyboard(binding) {
  const rows = [
    ...
    [{ text: "📊 Отчёты", callback_data: "pulse:totals" }],
  ];
  if (binding?.type === "union") {
...
function pulseWeeksKeyboard(binding, selectedMask = 0n) {
  ...
  rows.push([{ text: "⬅️ Отчёты", callback_data: "pulse:totals" }]);
  return { inline_keyboard: rows };
}
...
async function generatePulseTotalsImage({ owner, period, reportCount, metrics, total, serviceLabel }) {
  ...
  const svg = Buffer.from(`<svg width="1080" height="${height}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="62" y="142" fill="#99f6e4" font-family="Arial, DejaVu Sans, sans-serif" font-size="29">ОТЧЁТ ЗА ВЫБРАННЫЙ ПЕРИОД</text>
...
async function sendBoundClubCommands(chatId, binding) {
  if (binding.type === "union") {
    const lines = [
      ...
      "",
      "<b>Статистика</b>",
      "<b>/пульс</b> — динамика, игроки, отчёты, клубы и расписание турниров",
  ...
  const lines = [
    ...
    "",
    "<b>Статистика</b>",
    "<b>/пульс</b> — динамика, игроки и отчёты клуба",
