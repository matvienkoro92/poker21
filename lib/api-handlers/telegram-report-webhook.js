...
async function sendJackpot(chatId, messageId, data = latestUnionData) {
  ...
  const leagues = Array.isArray(unionJackpotSummary.leagues) ? unionJackpotSummary.leagues : [];
  ...
  const unclassifiedFee = Number(unionJackpotSummary.unclassifiedFee || 0);
  ...
  const totalFee = Number(unionJackpotSummary.totalFee ?? (regularFee + jackpot21Fee));
  const totalPayout = Number(unionJackpotSummary.totalPayout ?? (regularPayout + jackpot21Payout));
  ...
  lines.push(
    "",
    `Обычный джекпот (подтверждено) — ${formatRake(regularFee)}`,
    ...
    "",
    `Не разделено по типу у остальных лиг — ${formatRake(unclassifiedFee)}`,
    `Неразделённые выплаты — ${formatRake(unclassifiedPayout)}`,
    "",
