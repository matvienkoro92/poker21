...
async function sendOverview(chatId, data = latestUnionData) {
  const diamondSales = await getDiamondSales(data);
  // В сводке продажа алмазов показывается отдельным приходом. Доли здесь
  // считаем только от рейка, иначе алмазы визуально смешиваются с рейком.
  const { share } = calculateChineseRake(data, 0);
  ...
  // Продажа уже распределена между долями и поэтому не является ещё одним
  // слагаемым сверки: строка ниже показывает её отдельно справочно.
  ...
  const lines = [
    ...
    `3. Продажа алмазов: <b>+${formatRake(diamondSales)}</b>`,
