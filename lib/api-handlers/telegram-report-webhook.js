...
async function sendUnionReports(chatId, messageId, data = latestUnionData) {
  ...
  const romanSet = new Set(romanReports);
  const sergeyReports = reports.filter((report) => !romanSet.has(report));
  const groups = [
    { heading: "❗ ДЛЯ РОМАНА:", reports: romanReports },
    { heading: "❗ ДЛЯ СЕРГЕЯ:", reports: sergeyReports },
  ];
  for (const [groupIndex, group] of groups.entries()) {
    const media = group.reports.map((report, reportIndex) => {
      const metrics = report.metrics || {};
      const lines = [
        ...(reportIndex === 0 ? [`<b>${group.heading}</b>`, ""] : []),
        ...
        `Возврат джекпота: ${Number(metrics.jackpotRefund || 0) > 0 ? "+" : ""}${formatRake(metrics.jackpotRefund)}`,
        "",
        `<b>Итого к расчёту: ${formatRake(metrics.total)}</b>`,
      ];
      return {
        type: "photo",
        media: `${APP_ORIGIN}${report.imagePath}?v=refund-plus-1`,
        caption: lines.join("\n"),
        parse_mode: "HTML",
      };
    });
    const sent = await telegram("sendMediaGroup", {
      chat_id: chatId,
      media,
