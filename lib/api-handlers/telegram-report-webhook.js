...
async function sendBoundUnionPlayerTops(chatId, binding, type, data = latestUnionData, editMessageId = null, periodIndex = 0) {
  ...
  const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", {
    chat_id: chatId,
    ...(editMessageId ? { message_id: editMessageId, reply_markup: pulsePlayersKeyboard(binding, periodIndex) } : {}),
    text: lines.join("\n"),
    parse_mode: "HTML",
  });
  return Boolean(sent.ok);
...
function pulsePlayersKeyboard(binding, selectedPeriodIndex = 0) {
  const periods = availableBoundReportPeriods(binding);
  return { inline_keyboard: [
    [
      { text: "💰 Рейк", callback_data: `pulse:player:rake:${selectedPeriodIndex}` },
      { text: "📈 Плюс", callback_data: `pulse:player:plus:${selectedPeriodIndex}` },
      { text: "📉 Минус", callback_data: `pulse:player:minus:${selectedPeriodIndex}` },
    ],
    ...periods.map((period, index) => [{
      text: `${index === selectedPeriodIndex ? "✓ " : ""}${displayIso(period.startDate, false)}–${displayIso(period.endDate, false)}`,
      callback_data: `pulse:players:period:${index}`,
    }]),
    [{ text: "⬅️ Пульс", callback_data: "pulse:menu" }],
  ] };
}
...
async function sendPulseSubmenu(chatId, binding, type, messageId, selectedPeriodIndex = 0) {
  ...
  const sent = await telegram("editMessageText", {
    ...
    parse_mode: "HTML",
    reply_markup: isPlayers ? pulsePlayersKeyboard(binding, selectedPeriodIndex) : pulseTotalsKeyboard(binding),
  });
  return Boolean(sent.ok);
}
...
module.exports = async function handler(req, res) {
  ...
  if ((pulseMenuCallback || pulsePlayerCallback || pulsePlayerPeriodCallback || pulseTotalCallback || pulseWeeksCallback || pulseWeeksToggleCallback || pulseWeeksApplyCallback) && callbackQuery?.message?.chat?.id != null) {
    const actorChatId = String(callbackQuery.message.chat.id);
    await telegram("answerCallbackQuery", { callback_query_id: callbackQuery.id });
    const binding = await getClubBinding(actorChatId);
    if (!binding) {
    ...
    if (pulsePlayerPeriodCallback) {
      ...
      const sent = await sendPulseSubmenu(actorChatId, binding, "players", callbackQuery.message.message_id, periodIndex);
