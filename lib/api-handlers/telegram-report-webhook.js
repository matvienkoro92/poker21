...
function pulsePlayersKeyboard() {
  return { inline_keyboard: [
    ...
    [{ text: "⬅️ Пульс", callback_data: "pulse:week" }],
  ] };
}
...
module.exports = async function handler(req, res) {
  ...
  const pulseMenuCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:(players|totals)$/) : null;
  ...
  if (clubBinding && (!isMainReportChat(chatId) || mainBalanceOnlyCommand)) {
    ...
    if (isPulseCommand(message.text)) {
      const sent = await sendBindingPulse(chatId, clubBinding, message.message_id);
