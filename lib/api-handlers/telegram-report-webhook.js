...
async function sendTournamentSchedule(chatId, messageId) {
  ...
  const lines = rows.length
    ? ["<b>РАСПИСАНИЕ ТУРНИРОВ</b>", "", ...rows.map((row, index) => `<b>${index + 1}.</b> ${escapeTelegramHtml(row.text)}`)]
    : ["<b>РАСПИСАНИЕ ТУРНИРОВ</b>", "", "Расписание пока не заполнено."];
...
module.exports = async function handler(req, res) {
  ...
  if (scheduleCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    await telegram("sendMessage", {
      chat_id: actorChatId,
      text: action === "add"
        ? "Введите данные турнира одним сообщением. Можно использовать несколько строк.\n\nНапример:\nПятница, 20:00\nFriday Special\nВход 3 000 ₽ · GTD 100 000 ₽\n\nДля отмены: /отмена"
  ...
  if (isRedisConfigured() && !String(message.text || "").trim().startsWith("/")) {
    ...
    if (pending?.[0]?.result) {
      ...
      if (!state || !["add", "edit"].includes(state.action)) {
        ...
        const sent = await telegram("sendMessage", {
          chat_id: chatId,
          text: state.action === "edit" ? "✅ Турнир изменён.\n\nПоказать список: /расписание" : "✅ Турнир добавлен.\n\nПоказать список: /расписание",
          reply_to_message_id: message.message_id,
        });
