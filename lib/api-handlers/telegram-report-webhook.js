...
async function sendTournamentSchedule(chatId, messageId, scheduleChatId = chatId, viewMode = "all") {
  ...
  const lines = ["📋 <b>ОБЩЕЕ РАСПИСАНИЕ</b>"];
...
module.exports = async function handler(req, res) {
  ...
  if (isScheduleCommand(message.text)) {
    ...
    if (viewMode === "menu") {
      const sent = await telegram("sendMessage", {
        chat_id: chatId,
        text: "Выберите расписание:\n\n/расписание сегодня — только турниры на сегодня\n/расписание общее — полное расписание",
        reply_to_message_id: message.message_id,
      });
