...
async function acceptsPlayerSearchMessage(message) {
  ...
  if (!text || text.startsWith("/") || (!explicitReply && message.reply_to_message)) return false;
  if (!isRedisConfigured() || !message.from?.id) return explicitReply;
  const rows = await redisPipeline([["GETDEL", playerSearchPendingKey(message.chat.id, message.from.id)]], {
    context: "telegram-report.player-search.consume", timeoutMs: 2000,
  });
