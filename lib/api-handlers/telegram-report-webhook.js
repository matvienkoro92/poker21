...
function clubHistoryAnalysis(binding) {
  ...
  return {
    periods, players, cohorts, core, totalRake, coreRake: accumulated, periodStats,
...
async function sendClubCore(chatId, binding, messageId) {
  const analysis = clubHistoryAnalysis(binding);
  const rows = analysis.core.slice(0, 20);
  const lines = [
    `💎 <b>Ядро клуба — ${escapeTelegramHtml(binding.club)}</b>`,
    "Минимальная группа игроков, которая создала 70% рейка за всю доступную историю.",
    "",
    ...
    `Сейчас неактивны: <b>${analysis.core.filter((player) => !player.currentActive).length}</b>`,
    "",
    ...rows.map((player, index) => `${index + 1}. ${player.currentActive ? "🟢" : "🔴"} <b>${escapeTelegramHtml(player.nick)}</b> (${player.id})\n${formatRake(player.totalRake)} рейка · активных отчётов ${player.activeWeeks}/${analysis.periods.length} · последний ${displayIso(player.lastWeek.endDate)}`),
    ...(analysis.core.length > rows.length ? ["", `Показаны первые ${rows.length} из ${analysis.core.length}.`] : []),
  ];
  const sent = await telegram("editMessageText", {
    chat_id: chatId,
    ...
    reply_markup: { inline_keyboard: [
      [{ text: "📋 Всё ядро", callback_data: "pulse:analysis:list:core:0" }],
...
module.exports = async function handler(req, res) {
  ...
  const pulseAnalysisListCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:analysis:list:(inactive|recent|dormant|oneweek|new|core):(\d{1,3})$/) : null;
