...
async function sendUnionReports(chatId, messageId, data = latestUnionData) {
  ...
  for (const [groupIndex, group] of groups.entries()) {
    ...
    groupTotals.push({ recipient: group.recipient, reports: group.reports, total: groupTotal });
  ...
  const totalsSent = await telegram("sendMessage", {
    chat_id: chatId,
    text: groupTotals
      .map(({ recipient, reports: recipientReports, total }) => {
        const intermediate = recipientReports
          .map((report) => `${escapeTelegramHtml(report.league)}: ${formatRake(report.metrics?.total)}`)
          .join("\n");
        return `<b>${recipient}:</b>\n<b>ИТОГО: ${formatRake(total)}</b>\n${intermediate}`;
      })
      .join("\n\n"),
    parse_mode: "HTML",
  });
