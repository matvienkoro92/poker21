...
function expandScheduleRows(rows) {
  ...
  for (const row of rows) {
    ...
    let category = scheduleCategory(row.text);
    let buffer = [];
    const flush = () => {
    ...
    for (const line of lines) {
      if (/^турниры?\s+месяца[!,:.\s]*$/iu.test(line)) {
        flush();
        category = "month";
        continue;
      }
      if (/^турниры?\s+дня[!,:.\s]*$/iu.test(line)) {
        flush();
        category = "day";
        continue;
      }
      if (/^(?:ежедневные\s+турниры|ежедневно)[!,:.\s]*$/iu.test(line)) {
        flush();
        category = "daily";
        continue;
      }
...
async function sendTournamentSchedule(chatId, messageId) {
  const storedRows = await getTournamentSchedule(chatId);
  ...
  const lines = [
    ...
    ...(todayMain.length
      ...
      : []),
    ...(todayRegular.length
      ? [todayMain.length ? "" : "<b>СЕГОДНЯШНИЕ ТУРНИРЫ</b>", "", ...todayRegular.map((row, index) => scheduleEntryText(row, index))]
      : []),
  ...
  for (const category of categories) {
    ...
    lines.push("", category.title, "", ...categoryRows.map((row, index) => scheduleEntryText(row, index, category.key === "month")));
  }
