...
async function sendChineseRake(chatId, messageId, data = latestUnionData) {
  ...
  const lines = [
    ...rows.map((row) => `${formatPercent(row.percent)}% — ${escapeTelegramHtml(row.label)} — ${formatRake(row.rake)} — ${formatRake(row.amount)}`),
    "",
    `<b>ИТОГО: ${formatRake(total)}</b>`,
    "",
  ...
  const photo = sent.ok ? await telegram("sendPhoto", {
    chat_id: chatId,
    photo: `${APP_ORIGIN}/assets/reports/share/2026-08-03_2026-08-09.png?v=share-table-1`,
  }) : { ok: false };
  return Boolean(sent.ok && photo.ok);
}
...
async function sendCommands(chatId, messageId) {
  const lines = [
    ...
    "<b>/доля</b> — рейк союзов и распределение долей",
