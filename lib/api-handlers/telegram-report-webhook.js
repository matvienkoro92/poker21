...
function parseRomanSentInput(text) {
  const match = String(text || "").trim().match(/^(вика|аня)\s+([+-]?[\d\s\u00a0\u202f]+(?:[.,]\d{1,2})?)\s*(?:р|руб(?:лей|ля|ль)?|₽)?\s*$/iu);
  if (!match) return null;
  const value = Number(match[2].replace(/[\s\u00a0\u202f]/g, "").replace(",", "."));
  if (!Number.isFinite(value) || value <= 0) return null;
  return { recipient: match[1].toLowerCase() === "вика" ? "vika" : "anya", label: match[1].toLowerCase() === "вика" ? "Вике" : "Ане", cents: Math.round(value * 100) };
}
...
async function getEntityStoredBalance(type, entityId) {
  if (!isRedisConfigured()) return { cents: 0, usdCents: 0 };
  const keys = await scanRedisKeys("poker21:telegram-report:club-chat:*", "telegram-report.roman-total.bindings.scan");
  if (!keys.length) return { cents: 0, usdCents: 0 };
  const values = await redisPipeline(keys.map((key) => ["GET", key]), { context: "telegram-report.roman-total.bindings.get", timeoutMs: 4000 });
  const matches = [];
  for (let index = 0; index < keys.length; index += 1) {
    try {
      const binding = JSON.parse(String(values?.[index]?.result || ""));
      const boundId = type === "union" ? binding.leagueId : binding.clubId;
      if (binding.type === type && String(boundId) === String(entityId)) {
        matches.push(String(keys[index]).slice("poker21:telegram-report:club-chat:".length));
      }
    } catch (_) {}
  }
  if (!matches.length) return { cents: 0, usdCents: 0 };
  const balances = await redisPipeline(
    matches.flatMap((boundChatId) => [["GET", chatBalanceKey(boundChatId)], ["GET", chatBalanceUsdKey(boundChatId)]]),
    { context: "telegram-report.roman-total.balances", timeoutMs: 4000 },
  );
  for (let index = 0; index < matches.length; index += 1) {
    const cents = Number(balances?.[index * 2]?.result || 0);
    const usdCents = Number(balances?.[index * 2 + 1]?.result || 0);
    if (cents || usdCents) return { cents, usdCents };
  }
  return { cents: 0, usdCents: 0 };
}
...
async function sendRomanTotal(chatId, messageId, data = latestUnionData, requestInput = true) {
  const [mainBalance, ppcBalance, arenaBalance, sentValues] = await Promise.all([
    getChatBalance(chatId),
    getEntityStoredBalance("union", "259822"),
    getEntityStoredBalance("club", "600344"),
    redisPipeline([
  ...
  const ppcShare = Number(ppcBalance.cents || 0) / 100;
  const arenaShare = Number(arenaBalance.cents || 0) / 100;
  ...
  const lines = [
    ...
    `+ Доля PPC: <b>${formatRake(ppcShare)} ₽</b>`,
    `+ Доля PC Arena: <b>${formatRake(arenaShare)} ₽</b>`,
    ...
    `Отправлено Вике: <b>-${formatRake(sentVika)} ₽</b>`,
    `Отправлено Ане: <b>-${formatRake(sentAnya)} ₽</b>`, "",
    `<b>ИТОГО: ${formatRake(total)} ₽</b>`,
    `<b>ДОЛЛАРЫ: ${formatRake(usdBalance)} $</b>`,
  ];
  const sent = await telegram("sendMessage", { chat_id: chatId, text: lines.join("\n"), parse_mode: "HTML", reply_to_message_id: messageId });
  if (!requestInput || !sent.ok) return Boolean(sent.ok);
  const prompt = await telegram("sendMessage", {
    chat_id: chatId,
    text: "Введите, кому и сколько отправлено, одной строкой:\n\nВика 50000\nили\nАня 50000",
    reply_markup: { force_reply: true, selective: true, input_field_placeholder: "Вика 50000" },
  });
  return Boolean(prompt.ok);
}
...
async function telegram(method, body, retryCount = 0) {
  try {
    ...
    if (!result.ok && response.status === 429 && retryCount === 0 && retryAfter > 0 && retryAfter <= 15) {
    ...
    if (!result.ok) {
      console.error(`telegram-report-webhook: Telegram rejected ${method}`, response.status, result.description || "Unknown error");
    }
    return result;
  } catch (error) {
...
async function sendAllChatBalances(chatId, messageId) {
  ...
  const unrecordedLines = unrecorded.length
    ? unrecorded.map(formatUnrecordedBalanceOperation)
    : ["Нет новых операций."];
  const balanceLines = ["<b>Балансы союзов</b>", "", ...unionLines, "", "<b>Балансы клубов</b>", "", ...clubLines];
...
module.exports = async function handler(req, res) {
  if (req.method === "GET" && req.query?.repair === "webhook-20260818-2240-7c91") {
    if (!BOT_TOKEN || !WEBHOOK_SECRET) return res.status(500).json({ ok: false, error: "Bot webhook is not configured" });
    const repaired = await telegram("setWebhook", {
      url: `${APP_ORIGIN}/api/telegram-report-webhook`,
      secret_token: WEBHOOK_SECRET,
      allowed_updates: ["message", "edited_message", "callback_query"],
    });
    return res.status(repaired.ok ? 200 : 502).json({ ok: Boolean(repaired.ok), description: repaired.description || "" });
  }
  ...
  if (isMainReportChat(chatId) && isRedisConfigured() && !String(message.text || "").trim().startsWith("/")) {
    ...
    if (pending?.[0]?.result) {
      const input = parseRomanSentInput(message.text);
      if (!input) {
        const sent = await telegram("sendMessage", {
          chat_id: chatId,
          text: "Не удалось распознать запись. Введите одной строкой, например: Вика 50000 или Аня 50000.",
          reply_to_message_id: message.message_id,
        });
        return res.status(200).json({ ok: true, romanTotal: true, recorded: false, invalid: true, sent: Boolean(sent.ok) });
      }
      const entry = JSON.stringify({ recipient: input.recipient, cents: input.cents, actorId: String(message.from?.id || ""), timestamp: new Date().toISOString() });
      await redisPipeline([
        ["INCRBY", romanTotalSentKey(input.recipient), String(input.cents)],
      ...
      const confirmed = await telegram("sendMessage", { chat_id: chatId, text: `Зафиксировано: ${input.label} отправлено ${formatRake(input.cents / 100)} ₽.` });
      const summarySent = await sendRomanTotal(chatId, message.message_id, latestUnionData, false);
      return res.status(200).json({ ok: true, romanTotal: true, recorded: true, recipient: input.recipient, cents: input.cents, sent: Boolean(confirmed.ok && summarySent) });
    }
  }
  ...
  if (isRomanTotalCommand(message.text)) {
    ...
    await redisPipeline(
      [["SET", romanTotalPendingKey(chatId, message.from?.id), "1", "EX", "600"]],
      { context: "telegram-report.roman-total.pending.set", timeoutMs: 2000 },
    );
    const sent = await sendRomanTotal(chatId, message.message_id);
