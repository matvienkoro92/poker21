...
function pulseScheduleKeyboard(viewMode = "") {
  return { inline_keyboard: [
    ...
    [{ text: "⬅️ Назад", callback_data: "pulse:menu", style: "primary" }],
  ] };
}
...
function insightMenuKeyboard(includePulseBack = false) {
  return { inline_keyboard: [
    ...
    ...(includePulseBack ? [[{ text: "⬅️ Игроки", callback_data: "pulse:players" }]] : []),
  ] };
}
...
async function sendInsightPlayers(chatId, binding, kind, editMessageId, returnToPulse = false) {
  ...
  const sent = await telegram("editMessageText", {
    ...
    reply_markup: returnToPulse
      ? { inline_keyboard: [[{ text: "⬅️ Динамика", callback_data: "pulse:dynamics" }]] }
...
function pulseMainKeyboard(binding) {
  const rows = [
    [{ text: "👥 Игроки", callback_data: "pulse:players" }],
    [{ text: "📈 Динамика", callback_data: "pulse:dynamics" }],
    [{ text: "📊 Итоги", callback_data: "pulse:totals" }],
  ];
  if (binding?.type === "union") {
    rows.push([{ text: "🏢 Клубы союза", callback_data: "pulse:clubs" }]);
    rows.push([{ text: "📅 Расписание турниров", callback_data: "pulse:schedule" }]);
  }
  return { inline_keyboard: rows };
}

function pulseUnionClubsKeyboard() {
  return { inline_keyboard: [
    [{ text: "🏢 Клубы и рейк", callback_data: "pulse:unionclubs:list" }],
    [{ text: "🔎 Информация о клубе", callback_data: "pulse:unionclubs:info" }],
    [
      { text: "💰 Рейк", callback_data: "pulse:unionclubs:top:rake" },
      { text: "📈 Плюс", callback_data: "pulse:unionclubs:top:plus" },
      { text: "📉 Минус", callback_data: "pulse:unionclubs:top:minus" },
    ],
    [{ text: "⬅️ Назад", callback_data: "pulse:menu", style: "primary" }],
  ] };
}

function pulseUnionClubPeriodsKeyboard(binding, selectedIndex = 0) {
  ...
  return { inline_keyboard: [
    ...periods.map((period, index) => [{
      ...
      callback_data: `pulse:unionclubs:period:${index}`,
      ...(index === selectedIndex ? { style: "success" } : {}),
    }]),
    [{ text: "⬅️ Назад", callback_data: "pulse:clubs", style: "primary" }],
  ] };
}
...
function pulseTotalsKeyboard(binding) {
  return { inline_keyboard: [
    [{ text: "Выбрать период", callback_data: "pulse:total:choose" }],
    [{ text: "Всё время", callback_data: "pulse:total:all" }],
    [{ text: "⬅️ Назад", callback_data: "pulse:menu", style: "primary" }],
  ] };
}
...
function pulseWeeksKeyboard(binding, selectedMask = 0n) {
  ...
  const rows = reports.map((report, index) => {
    ...
    return [{
      text: `${selected ? "✅" : "▫️"} Неделя ${index + 1} · ${displayIso(report.startDate, false)}–${displayIso(report.endDate, false)}`,
      callback_data: `pulse:total:toggle:${index}:${selectedMask.toString(16)}`,
    }];
  });
  const selectedCount = reports.filter((_, index) => (selectedMask & (1n << BigInt(index))) !== 0n).length;
  if (selectedCount) rows.push([{
    text: `Показать выбранные (${selectedCount})`,
    callback_data: `pulse:total:apply:${selectedMask.toString(16)}`,
  }]);
  rows.push([{ text: "⬅️ Итоги", callback_data: "pulse:totals" }]);
  return { inline_keyboard: rows };
}

async function sendPulseWeeksMenu(chatId, binding, messageId, selectedMask = 0n) {
  const count = availableConsecutiveReportWeeks(binding);
  const sent = await telegram(messageId ? "editMessageText" : "sendMessage", {
    chat_id: chatId,
    ...(messageId ? { message_id: messageId } : {}),
    text: count
      ? `Выберите нужные недели и нажмите «Показать выбранные».\n\nДоступно последовательных отчётов: ${count}.`
...
async function sendPulseSubmenu(chatId, binding, type, messageId, selectedMetric = "", selectedMask = 1n) {
  ...
  const sent = await telegram("editMessageText", {
    ...
    text: isPlayers
      ...
      : `📊 <b>Итоги — ${escapeTelegramHtml(binding.type === "union" ? binding.league : binding.club)}</b>\n\nКакой период показать?`,
    parse_mode: "HTML",
...
async function sendPulseMainMenu(chatId, binding, messageId = null, editMessageId = null) {
  ...
  const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", {
    ...
    text: `<b>💓 Пульс ${binding.type === "union" ? "союза" : "клуба"} — ${escapeTelegramHtml(owner)}</b>\n\nЧто показать?`,
    parse_mode: "HTML",
    reply_markup: pulseMainKeyboard(binding),
  });
  return Boolean(sent.ok);
}
...
async function sendPulseUnionClubsMenu(chatId, binding, messageId) {
  const sent = await telegram("editMessageText", {
    ...
    parse_mode: "HTML",
    reply_markup: pulseUnionClubsKeyboard(),
  });
  return Boolean(sent.ok);
}
...
async function sendPulseScheduleMenu(chatId, binding, messageId) {
  const sent = await telegram("editMessageText", {
    chat_id: chatId,
    message_id: messageId,
    text: `📅 <b>Расписание турниров — ${escapeTelegramHtml(binding.league)}</b>\n\nЧто показать?`,
    parse_mode: "HTML",
...
async function sendPulseUnionClubInfoMenu(chatId, binding, messageId, data = latestUnionData) {
  ...
  const sent = await telegram("editMessageText", {
    ...
    reply_markup: { inline_keyboard: [
      ...clubs.slice(0, 90).map((club) => [{
        text: String(club.club || club.clubId || "Клуб"),
        callback_data: `pulse:unionclubs:club:${club.clubId}`,
      }]),
      [{ text: "⬅️ Назад", callback_data: "pulse:clubs", style: "primary" }],
    ] },
  });
...
async function sendBindingPulse(chatId, binding, messageId, mode = "week", editMessageId = null) {
  ...
  const owner = binding.type === "union" ? binding.league : binding.club;
  const lines = [
    `<b>💓 Пульс ${binding.type === "union" ? "союза" : "клуба"} — ${escapeTelegramHtml(owner)}</b>`,
...
async function sendBoundUnionClubInfo(chatId, messageId, binding, query, data = latestUnionData, editMessageId = null) {
  ...
  if (!matches.length) {
    const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", {
      ...
      ...(!editMessageId && messageId ? { reply_to_message_id: messageId } : {}),
      ...(editMessageId ? { reply_markup: pulseUnionClubsKeyboard() } : {}),
    });
    return Boolean(sent.ok);
  }
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    const sent = await telegram(index === 0 && editMessageId ? "editMessageText" : "sendMessage", {
      ...
      ...(index === 0 && editMessageId ? { reply_markup: pulseUnionClubsKeyboard() } : {}),
    });
    allSent = allSent && Boolean(sent.ok);
  }
  return allSent;
...
async function generatePulseTotalsImage({ owner, period, reportCount, metrics, total, serviceLabel }) {
  ...
  const rowSvg = rows.map(([label, value], index) => {
    ...
    return `<g>
      ...
      <text x="86" y="${y}" fill="#d1fae5" font-family="Arial, DejaVu Sans, sans-serif" font-size="28">${escapeSvgText(label)}</text>
      <text x="994" y="${y}" text-anchor="end" fill="${color}" font-family="Arial, DejaVu Sans, sans-serif" font-size="30" font-weight="700">${escapeSvgText(formatRub(amount))}</text>
    </g>`;
  }).join("");
  ...
  const svg = Buffer.from(`<svg width="1080" height="${height}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="62" y="92" fill="#ffffff" font-family="Arial, DejaVu Sans, sans-serif" font-size="42" font-weight="700">${escapeSvgText(owner)}</text>
    <text x="62" y="142" fill="#99f6e4" font-family="Arial, DejaVu Sans, sans-serif" font-size="29">ИТОГИ ЗА ВЫБРАННЫЙ ПЕРИОД</text>
    <text x="62" y="190" fill="#d1fae5" font-family="Arial, DejaVu Sans, sans-serif" font-size="31">${escapeSvgText(period)}</text>
    <text x="1018" y="190" text-anchor="end" fill="#94a3b8" font-family="Arial, DejaVu Sans, sans-serif" font-size="25">Отчётов: ${reportCount}</text>
    ...
    <text x="62" y="${totalY + 18}" fill="#ffffff" font-family="Arial, DejaVu Sans, sans-serif" font-size="34" font-weight="700">ИТОГО К РАСЧЁТУ</text>
    <text x="1018" y="${totalY + 18}" text-anchor="end" fill="${Number(total || 0) >= 0 ? "#6ee7b7" : "#fb7185"}" font-family="Arial, DejaVu Sans, sans-serif" font-size="38" font-weight="700">${escapeSvgText(formatRub(total))}</text>
  </svg>`);
  return sharp(svg).png({ compressionLevel: 9 }).toBuffer();
}

async function sendTelegramPhotoBuffer(chatId, imageBuffer, caption, replyMarkup) {
  try {
    const form = new FormData();
    form.append("chat_id", String(chatId));
    form.append("photo", new Blob([imageBuffer], { type: "image/png" }), "pulse-totals.png");
...
async function sendTotal(chatId, messageId, command, servicePercent, clubId = null, editMessageId = null, binding = null) {
  ...
  if (command.type === "all") {
    ...
    if (!selectedReports.length) {
      const sent = await telegram("sendMessage", { chat_id: chatId, text: "Выбранные отчёты не найдены.", reply_to_message_id: messageId });
      return Boolean(sent.ok);
    }
    period = {
  ...
  if (binding) {
    const owner = binding.type === "union" ? binding.league : binding.club;
    try {
      const periodText = `${displayIso(period.startDate)}–${displayIso(effectiveEndDate)}`;
      const image = await generatePulseTotalsImage({
        owner,
        period: periodText,
      ...
      if (editMessageId) await telegram("deleteMessage", { chat_id: chatId, message_id: editMessageId });
      const uploaded = await sendTelegramPhotoBuffer(
        chatId,
        image,
        `<b>${escapeTelegramHtml(owner)}</b>\nИтоги за период: ${periodText}\nУчтено отчётов: ${reports.length}`,
        pulseTotalsKeyboard(binding),
      );
      if (uploaded.ok) return true;
      const fallback = await telegram("sendMessage", {
        chat_id: chatId,
        text: lines.join("\n"),
        parse_mode: "HTML",
      ...
      console.error("telegram-report-webhook: pulse totals image failed", error?.message || error);
    }
  }
  const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", {
    chat_id: chatId,
    ...(editMessageId ? { message_id: editMessageId, reply_markup: pulseTotalsKeyboard(binding || { type: "club" }) } : { reply_to_message_id: messageId }),
    text: lines.join("\n"),
    parse_mode: "HTML",
  });
  return Boolean(sent.ok);
...
function balanceMenuKeyboard() {
  return {
    inline_keyboard: [
      ...
      [{ text: "✏️ Установить баланс", callback_data: "balmenu:set" }],
      [
...
async function sendChatBalanceHistory(chatId, binding, editMessageId = null) {
  ...
  const sent = await telegram(editMessageId ? "editMessageText" : "sendMessage", {
    ...
    text: lines.join("\n"),
    parse_mode: "HTML",
    ...(editMessageId ? { reply_markup: { inline_keyboard: [[{ text: "⬅️ Баланс", callback_data: "balmenu:show" }]] } } : {}),
  });
  return Boolean(sent.ok);
}
...
async function sendPaymentDetailsRegistry(chatId, allowActions = true, messageId = null) {
  ...
  if (!active.length) {
    ...
    ].join("\n"), allowActions ? paymentDetailsMenuButtons() : [], messageId);
  }
  ...
  if (allowActions) keyboard.push(...paymentDetailsMenuButtons());
...
async function sendAllPaymentBalances(chatId, messageId) {
  ...
  for (let index = 0; index < keys.length; index += 1) {
    try {
      const binding = JSON.parse(String(bindingsResult?.[index]?.result || ""));
      const name = binding.type === "union" ? binding.league : binding.club;
      if (!name || isHiddenBalanceBinding(binding)) continue;
      entries.push({
...
async function sendBoundClubCommands(chatId, binding) {
  if (binding.type === "union") {
    const lines = [
      ...
      "",
      "<b>Отчёты</b>",
      "<b>/отчеты</b> — выбрать отчёт союза по периоду",
      "",
      "<b>Статистика</b>",
      "<b>/пульс</b> — динамика, игроки, итоги, клубы и расписание турниров",
      "<b>/гонка клубов</b> — рейтинг клубов союза и разрывы по рейку",
      "",
      "<b>Баланс</b>",
      "<b>/баланс</b> — баланс, история и управление",
      "",
      "<b>Реквизиты</b>",
  ...
  const lines = [
    ...
    "",
    "<b>Отчёты</b>",
    "<b>/отчеты</b> — выбрать отчёт клуба по периоду",
    "",
    "<b>Баланс</b>",
    "<b>/баланс</b> — баланс, история и управление",
    "",
    "<b>Статистика</b>",
    "<b>/пульс</b> — динамика, игроки и итоги клуба",
...
module.exports = async function handler(req, res) {
  ...
  const pulseMenuCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:(menu|dynamics|players|totals|clubs|schedule)$/) : null;
  ...
  const pulsePlayerSelectCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:player:select:(rake|plus|minus):([0-9a-f]+)$/) : null;
  const pulsePlayerToggleCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:players:toggle:(\d{1,2}):(none|rake|plus|minus):([0-9a-f]+)$/) : null;
  ...
  const pulseUnionClubsPeriodCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:unionclubs:period:(\d{1,2})$/) : null;
  const pulseUnionClubTopCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:unionclubs:top:(rake|plus|minus)$/) : null;
  const pulseUnionClubInfoCallback = callbackQuery ? String(callbackQuery.data || "").match(/^pulse:unionclubs:club:(\d+)$/) : null;
  ...
  const balanceMenuCallback = callbackQuery ? String(callbackQuery.data || "").match(/^balmenu:(show|history|set|add|subtract)$/) : null;
  ...
  if (reportsPeriodCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    if (!canEditReport) await telegram("deleteMessage", { chat_id: actorChatId, message_id: callbackQuery.message.message_id });
  ...
  if (balanceMenuCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    if (action === "history") {
      const sent = await sendChatBalanceHistory(actorChatId, binding, callbackQuery.message.message_id);
      return res.status(200).json({ ok: true, balanceMenu: action, sent });
    }
    const instructions = {
  ...
  if ((pulseMenuCallback || pulsePlayerCallback || pulsePlayerPeriodCallback || pulsePlayerSelectCallback || pulsePlayerToggleCallback || pulseTotalCallback || pulseWeeksCallback || pulseWeeksToggleCallback || pulseWeeksApplyCallback || pulseUnionClubsCallback || pulseUnionClubsPeriodCallback || pulseUnionClubTopCallback || pulseUnionClubInfoCallback || pulseScheduleViewCallback) && callbackQuery?.message?.chat?.id != null) {
    const actorChatId = String(callbackQuery.message.chat.id);
    await telegram("answerCallbackQuery", { callback_query_id: callbackQuery.id });
    const binding = await getClubBinding(actorChatId);
    if (!binding) {
    ...
    if (pulseMenuCallback) {
      ...
      if (section === "menu") {
        ...
        if (hasPhoto) await telegram("deleteMessage", { chat_id: actorChatId, message_id: callbackQuery.message.message_id });
        const sent = await sendPulseMainMenu(actorChatId, binding, null, hasPhoto ? null : callbackQuery.message.message_id);
      ...
      if (section === "schedule") {
        const sent = binding.type === "union"
          ? await sendPulseScheduleMenu(actorChatId, binding, callbackQuery.message.message_id)
          : await sendPulseMainMenu(actorChatId, binding, null, callbackQuery.message.message_id);
        return res.status(200).json({ ok: true, pulseMenu: section, sent });
      }
    ...
    if (binding.type === "union" && pulseUnionClubsCallback) {
      ...
      const periods = availableBoundReportPeriods(binding).filter((period) => period.leaguePlayerTops);
      const periodData = periods[0] || latestUnionData;
      const sent = await sendBoundUnionClubs(actorChatId, binding, periodData, callbackQuery.message.message_id, pulseUnionClubPeriodsKeyboard(binding, 0));
      return res.status(200).json({ ok: true, pulseUnionClubs: "list", sent });
    }
    if (binding.type === "union" && pulseUnionClubsPeriodCallback) {
      const periods = availableBoundReportPeriods(binding).filter((period) => period.leaguePlayerTops);
      const periodIndex = Math.min(Number(pulseUnionClubsPeriodCallback[1]), Math.max(0, periods.length - 1));
      const sent = await sendBoundUnionClubs(actorChatId, binding, periods[periodIndex] || latestUnionData, callbackQuery.message.message_id, pulseUnionClubPeriodsKeyboard(binding, periodIndex));
      return res.status(200).json({ ok: true, pulseUnionClubs: "period", periodIndex, sent });
    }
    ...
    if (binding.type === "union" && pulseUnionClubInfoCallback) {
      const sent = await sendBoundUnionClubInfo(actorChatId, null, binding, pulseUnionClubInfoCallback[1], latestUnionData, callbackQuery.message.message_id);
      return res.status(200).json({ ok: true, pulseUnionClubs: "club", clubId: pulseUnionClubInfoCallback[1], sent });
    }
    if (binding.type === "union" && pulseScheduleViewCallback) {
      const sent = await sendTournamentSchedule(
        actorChatId,
        null,
        actorChatId,
    ...
    if (pulseTotalCallback?.[1] === "choose") {
      const hasPhoto = Boolean(callbackQuery.message.photo?.length);
      if (hasPhoto) await telegram("deleteMessage", { chat_id: actorChatId, message_id: callbackQuery.message.message_id });
      const sent = await sendPulseWeeksMenu(actorChatId, binding, hasPhoto ? null : callbackQuery.message.message_id);
    ...
    if (pulseWeeksToggleCallback) {
      ...
      const sent = await sendPulseWeeksMenu(actorChatId, binding, callbackQuery.message.message_id, mask);
    ...
    if (pulseWeeksApplyCallback) {
      ...
      if (!endDates.length) {
        const sent = await sendPulseWeeksMenu(actorChatId, binding, callbackQuery.message.message_id);
        return res.status(200).json({ ok: true, pulseTotal: false, selected: 0, sent });
      }
      const sent = await sendTotal(actorChatId, null, { type: "selected", endDates }, DEFAULT_SERVICE_PERCENT, binding.type === "club" ? binding.clubId : null, callbackQuery.message.message_id, binding);
    ...
    const command = totalType === "weeks" ? { type: "weeks", count: selectedWeeks }
      : totalType === "month" ? { type: "previousMonth" }
        : { type: "all" };
    const sent = await sendTotal(actorChatId, null, command, DEFAULT_SERVICE_PERCENT, binding.type === "club" ? binding.clubId : null, callbackQuery.message.message_id, binding);
  ...
  if (paymentMenuCallback && callbackQuery?.message?.chat?.id != null) {
    ...
    if (action === "place") {
      ...
      return res.status(200).json({ ok: true, paymentMenu: action, sent: Boolean(sent.ok) });
    }
    if (action === "remove") {
  ...
  if (clubBinding && (!isMainReportChat(chatId) || mainBalanceOnlyCommand)) {
    ...
    if (isReportsMenuCommand(message.text)) {
      const sent = await sendBoundReportsMenu(chatId, clubBinding, message.message_id);
      return res.status(200).json({ ok: true, clubMode: true, reports: true, sent });
    }
