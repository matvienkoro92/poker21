...
function parseBalanceCommand(text) {
  ...
  const reduceMatch = payload.replace(/[\s\u00a0\u202f]/g, "").match(/^сократить(?:покурсу)?(\d+(?:[.,]\d{1,4})?)$/iu);
  if (reduceMatch) {
...
function reducedBalanceValues(balance, rate) {
  ...
  if (rubAbs >= usdInRubCents) {
    return { cents: Math.sign(rub) * (rubAbs - usdInRubCents), usdCents: 0 };
  }
  const rubInUsdCents = Math.min(usdAbs, Math.round(rubAbs / rate));
  return { cents: 0, usdCents: Math.sign(usd) * (usdAbs - rubInUsdCents) };
}
  ...
  if (clubBinding && (!isMainReportChat(chatId) || mainBalanceOnlyCommand)) {
    ...
    if (balanceCommand) {
      ...
      if (balanceCommand.action === "invalid") {
        const sent = await telegram("sendMessage", { chat_id: chatId, text: "Укажите валюту: /баланс 300р, 40$, /баланс +50$ или /баланс -100р. Для взаимного погашения валют: /баланс сократить 92,5" });
