...
async function sendInsightPlayers(chatId, binding, kind, messageId) {
  const configs = {
...
function insightPulseMetrics(period, binding) {
  ...
  if (binding.type === "union") {
    const league = (period.leaguePlayerTops?.leagues || []).find((row) => String(row.leagueId) === String(binding.leagueId));
    const clubIds = new Set((league?.clubs || []).map((club) => String(club.clubId || club.id || "")));
    const directoryClubs = (period.directory?.clubs || []).filter((club) => clubIds.has(String(club.id)));
    return {
      activePlayers: activeRows.length,
      rake: activeRows.reduce((sum, row) => sum + row.rake, 0),
      hands: directoryClubs.reduce((sum, club) => sum + Number(club.hands || 0), 0),
      activeClubs: (league?.clubs || []).filter((club) => Number(club.rake || 0) !== 0 || (club.playerRows || []).some(insightPlayerActive)).length,
    };
  }
  const club = (period.directory?.clubs || []).find((row) => String(row.id) === String(binding.clubId));
  return { activePlayers: activeRows.length, rake: Number(club?.rake || 0), hands: Number(club?.hands || 0) };
}

function pulseDelta(current, previous, formatter = formatInteger) {
  const delta = Number(current || 0) - Number(previous || 0);
  const percent = Number(previous || 0) ? delta / Math.abs(Number(previous)) * 100 : null;
  return `${formatter(current)} <b>${delta >= 0 ? "▲" : "▼"} ${formatter(Math.abs(delta))}${percent == null ? "" : ` (${Math.abs(percent).toLocaleString("ru-RU", { maximumFractionDigits: 0 })}%)`}</b>`;
}

async function sendBindingPulse(chatId, binding, messageId) {
  const periods = insightPeriods();
  const latest = periods[0];
  const previous = periods[1];
  if (!latest || !previous) {
    const sent = await telegram("sendMessage", { chat_id: chatId, text: "Для пульса нужны как минимум два недельных отчёта." });
    return Boolean(sent.ok);
  }
  ...
  const lines = [
    ...
    "",
    `Активные игроки: ${pulseDelta(current.activePlayers, prior.activePlayers)}`,
    `Рейк: ${pulseDelta(current.rake, prior.rake, formatRake)}`,
    `Раздачи: ${pulseDelta(current.hands, prior.hands)}`,
    ...(binding.type === "union" ? [`Активные клубы: ${pulseDelta(current.activeClubs, prior.activeClubs)}`] : []),
  ];
  const sent = await telegram("sendMessage", { chat_id: chatId, text: lines.join("\n"), parse_mode: "HTML", reply_to_message_id: messageId });
  return Boolean(sent.ok);
}
...
async function sendBoundClubCommands(chatId, binding) {
  if (binding.type === "union") {
    const lines = [
      ...
      "<b>/пульс</b> — состояние союза и динамика за неделю",
      "<b>/игроки клуба</b> — новички, уснувшие, вернувшиеся и стабильные игроки",
  ...
  const lines = [
    ...
    "<b>/расписание общее</b> — полное расписание турниров",
    "<b>/поменять расписание</b> — добавить, изменить или удалить турнир",
    "<b>/пульс</b> — состояние клуба и динамика за неделю",
    "<b>/игроки клуба</b> — новички, уснувшие, вернувшиеся и стабильные игроки",
