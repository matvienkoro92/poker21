/**
 * Чат: общий для всех + личные сообщения + групповые чаты.
 ...
 *   poker_app:group_msg:{id}, poker_app:group_members:{id}, poker_app:group_meta:{id}, poker_app:user_groups:{tg_id}.
 */
...
const GROUP_MSG_PREFIX = "poker_app:group_msg:";
const GROUP_MEMBERS_PREFIX = "poker_app:group_members:";
const GROUP_META_PREFIX = "poker_app:group_meta:";
const USER_GROUPS_PREFIX = "poker_app:user_groups:";
const MAX_GROUP_MEMBERS = 50;

function groupMsgKey(gid) {
  return GROUP_MSG_PREFIX + gid;
}
function groupMembersKey(gid) {
  return GROUP_MEMBERS_PREFIX + gid;
}
function groupMetaKey(gid) {
  return GROUP_META_PREFIX + gid;
}
function userGroupsKey(userId) {
  return USER_GROUPS_PREFIX + userId;
}
function isValidGroupId(gid) {
  return typeof gid === "string" && /^grp_[a-f0-9]{18}$/.test(gid);
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "DELETE") {
    ...
    const delGroupId = body.groupId || req.query.groupId;
    if (!messageId) return res.status(400).json({ ok: false, error: "messageId обязателен" });

    let redisKey = GENERAL_KEY;
    if (delGroupId && isValidGroupId(String(delGroupId))) {
      const gid = String(delGroupId);
      const mem = await redisPipeline([["SISMEMBER", groupMembersKey(gid), myId]]);
      if (!mem || mem[0]?.result !== 1) return res.status(403).json({ ok: false, error: "Нет доступа к чату" });
      redisKey = groupMsgKey(gid);
    } else if (withId) {
      redisKey = convKey(myId, withId.startsWith("tg_") ? withId : "tg_" + withId);
    }
  ...
  if (req.method === "PATCH") {
    ...
    if (action === "edit") {
      ...
      const withId = body.with || body.conversationWith || req.query.with;
      const editGroupId = body.groupId || req.query.groupId;
      if (!messageId) return res.status(400).json({ ok: false, error: "messageId обязателен" });
      if (!newText || newText.length > 500) return res.status(400).json({ ok: false, error: "Текст от 1 до 500 символов" });
      let redisKey = GENERAL_KEY;
      if (editGroupId && isValidGroupId(String(editGroupId))) {
        const gid = String(editGroupId);
        const mem = await redisPipeline([["SISMEMBER", groupMembersKey(gid), myId]]);
        if (!mem || mem[0]?.result !== 1) return res.status(403).json({ ok: false, error: "Нет доступа к чату" });
        redisKey = groupMsgKey(gid);
      } else if (withId) {
    ...
    if (action === "reaction") {
      ...
      const withId = body.with || body.conversationWith || req.query.with;
      const reactGroupId = body.groupId || req.query.groupId;
      const allowedEmojis = ["👍", "❤️", "😂", "😮", "😢", "🙏"];
      if (!messageId) return res.status(400).json({ ok: false, error: "messageId обязателен" });
      if (!allowedEmojis.includes(emoji)) return res.status(400).json({ ok: false, error: "Недопустимая реакция" });
      let redisKey = GENERAL_KEY;
      if (reactGroupId && isValidGroupId(String(reactGroupId))) {
        const gid = String(reactGroupId);
        const mem = await redisPipeline([["SISMEMBER", groupMembersKey(gid), myId]]);
        if (!mem || mem[0]?.result !== 1) return res.status(403).json({ ok: false, error: "Нет доступа к чату" });
        redisKey = groupMsgKey(gid);
      } else if (withId) {
  ...
  if (req.method === "GET") {
    ...
    const groupQ = req.query.group || req.query.groupId;

    if (groupQ && isValidGroupId(String(groupQ))) {
      const gid = String(groupQ);
      const memChk = await redisPipeline([["SISMEMBER", groupMembersKey(gid), myId]]);
      if (!memChk || memChk[0]?.result !== 1) {
        return res.status(403).json({ ok: false, error: "Нет доступа к чату" });
      }
      const now = Date.now();
      const minScore = now - ONLINE_TTL_MS;
      const results = await redisPipeline([
        ["LRANGE", groupMsgKey(gid), "0", String(MAX_MESSAGES - 1)],
        ["ZADD", CHAT_ONLINE_KEY, String(now), myId],
        ["ZREMRANGEBYSCORE", CHAT_ONLINE_KEY, "-inf", String(minScore)],
        ["GET", groupMetaKey(gid)],
        ["SMEMBERS", groupMembersKey(gid)],
      ]);
      const raw = results && results[0] && results[0].result !== undefined ? results[0].result : [];
      ...
      let title = "Группа";
      try {
        const metaRaw = results[3]?.result;
        if (metaRaw && typeof metaRaw === "string") {
          const o = JSON.parse(metaRaw);
          if (o && o.title) title = String(o.title).slice(0, 60);
        }
      } catch (e) {}
      let memberIds = Array.isArray(results[4]?.result) ? results[4].result : [];
      memberIds = [...new Set(memberIds.map((x) => String(x)))].filter((id) => id.startsWith("tg_"));
      const fromIds = [...new Set(deduped.map((m) => m.from).filter(Boolean))];
      const participantsCount = memberIds.length;
      let onlineCount = 0;
      if (memberIds.length > 0) {
        const scoreCmds = memberIds.flatMap((id) => [["ZSCORE", CHAT_ONLINE_KEY, id]]);
      ...
      const nameCmds = memberIds.length > 0 ? memberIds.map((id) => ["HGET", "poker_app:visitor_usernames", id]) : [];
      const nameRes = nameCmds.length ? await redisPipeline(nameCmds) : [];
      const members = memberIds.map((id, i) => {
        const un = nameRes && nameRes[i] && nameRes[i].result ? String(nameRes[i].result).trim() : "";
        return { id, name: un ? "@" + un : id.replace(/^tg_/, "ID ") };
      });
    ...
    const partnerIds = partners.filter((id) => id.startsWith("tg_") && id !== myId && !isAdmin(id));
    ...
    const adminIds = ADMIN_IDS.map((id) => (id.startsWith("tg_") ? id : "tg_" + id));
    const ugRes = await redisPipeline([["SMEMBERS", userGroupsKey(myId)]]);
    const groupIds = Array.isArray(ugRes?.[0]?.result)
      ? [...new Set(ugRes[0].result.map((x) => String(x)))].filter((g) => isValidGroupId(g))
      : [];
    ...
    groupIds.forEach((gid) => {
      lrangeCmds.push(["LRANGE", groupMsgKey(gid), "0", String(MAX_MESSAGES - 1)]);
    });
    lrangeCmds.push(["LRANGE", GENERAL_KEY, "0", String(MAX_MESSAGES - 1)]);
    const lrangeResults = await redisPipeline(lrangeCmds);
    const groupMetaCmds = [];
    groupIds.forEach((gid) => {
      groupMetaCmds.push(["GET", groupMetaKey(gid)]);
      groupMetaCmds.push(["SCARD", groupMembersKey(gid)]);
    });
    const groupMetaResults = groupMetaCmds.length > 0 ? await redisPipeline(groupMetaCmds) : [];
    ...
    if (lrangeResults && Array.isArray(lrangeResults)) {
      if (allIdsForUnread.length > 0) {
        allIdsForUnread.forEach((id, i) => {
          ...
          const list = Array.isArray(raw) ? raw : [];
          const lastView = lastViewed[id] != null ? String(lastViewed[id]) : "";
          let count = 0;
          list.forEach((s) => {
            try {
              const m = typeof s === "string" ? JSON.parse(s) : null;
              if (m && (m.time || "") > lastView && m.from === id) count++;
            } catch (e) {}
          });
      ...
      groupIds.forEach((gid, gi) => {
        const i = allIdsForUnread.length + gi;
        const raw = lrangeResults[i]?.result;
        const list = Array.isArray(raw) ? raw : [];
        const lastView = lastViewed[gid] != null ? String(lastViewed[gid]) : "";
        let count = 0;
        list.forEach((s) => {
          try {
            const m = typeof s === "string" ? JSON.parse(s) : null;
            if (m && (m.time || "") > lastView && m.from !== myId) count++;
          } catch (e) {}
        });
        unreadCounts[gid] = count;
      });
      ...
      const lastViewGeneral = lastViewed.general != null ? String(lastViewed.general) : "";
      generalList.forEach((s) => {
        try {
          const m = typeof s === "string" ? JSON.parse(s) : null;
          if (m && (m.time || "") > lastViewGeneral && m.from !== myId) generalUnreadCount++;
        } catch (e) {}
      });
    }
    ...
    if (lrangeResults && Array.isArray(lrangeResults) && groupIds.length > 0) {
      groupIds.forEach((gid, gi) => {
        const i = allIdsForUnread.length + gi;
        const raw = lrangeResults[i]?.result;
        const list = Array.isArray(raw) ? raw : [];
        ...
        if (first != null) {
          try {
            const m = typeof first === "string" ? JSON.parse(first) : first;
            if (m && m.time) lastMessageTime[gid] = String(m.time);
          } catch (e) {}
        }
      });
    }

    const groupsList = groupIds.map((gid, gi) => {
      let title = "Группа";
      try {
        const metaRaw = groupMetaResults && groupMetaResults[gi * 2] ? groupMetaResults[gi * 2].result : null;
        if (metaRaw && typeof metaRaw === "string") {
          const o = JSON.parse(metaRaw);
          if (o && o.title) title = String(o.title).slice(0, 60);
        }
      ...
      let memberCount = 0;
      const sc = groupMetaResults && groupMetaResults[gi * 2 + 1] ? groupMetaResults[gi * 2 + 1].result : null;
      if (sc != null) memberCount = parseInt(String(sc), 10) || 0;
      const idx = allIdsForUnread.length + gi;
      const raw = lrangeResults && lrangeResults[idx] ? lrangeResults[idx].result : [];
      const list = Array.isArray(raw) ? raw : [];
      let preview = "";
      if (list.length > 0) {
        try {
          const m = typeof list[0] === "string" ? JSON.parse(list[0]) : null;
          if (m) {
            const name = (m.fromName || "Игрок").trim();
            if (m.image) preview = name + ": [Фото]";
            else if (m.voice) preview = name + ": [Голосовое]";
            else if (m.document) preview = name + ": [Документ]";
            else if (m.text) preview = name + ": " + String(m.text).trim().replace(/\s+/g, " ").slice(0, 50);
      ...
      return {
        ...
        unreadCount: unreadCounts[gid] != null ? unreadCounts[gid] : 0,
        lastMessageTime: lastMessageTime[gid] || "",
      };
    });
    groupsList.sort((a, b) => {
    ...
    return res.status(200).json({
      ...
      generalUnreadCount: generalUnreadCount > 0 ? generalUnreadCount : 0,
    });
  }

  // POST: создание / вступление / приглашение в группу (без текста сообщения)
  const postAction = body.action && String(body.action);
  if (req.method === "POST" && postAction === "groupCreate") {
    const memberRaw = (body.memberId || body.member_id || body.userId || body.to || "").toString().trim();
    const title = (body.title || "").toString().trim().slice(0, 60) || "Группа";
    const gid = "grp_" + crypto.randomBytes(9).toString("hex");
    const meta = JSON.stringify({ title, createdBy: myId, createdAt: new Date().toISOString() });
    let otherId = null;
    if (memberRaw) {
      otherId = memberRaw.startsWith("tg_") ? memberRaw : "tg_" + memberRaw.replace(/^tg_/i, "");
      if (!otherId.startsWith("tg_") || otherId.length < 5) {
        return res.status(400).json({ ok: false, error: "Некорректный ID игрока" });
      }
      if (otherId === myId) return res.status(400).json({ ok: false, error: "Нельзя добавить себя" });
    }
    const cmds = otherId
      ? [
          ["SADD", groupMembersKey(gid), myId, otherId],
          ["SADD", userGroupsKey(myId), gid],
          ["SADD", userGroupsKey(otherId), gid],
          ["SET", groupMetaKey(gid), meta],
        ]
      : [
          ["SADD", groupMembersKey(gid), myId],
          ["SADD", userGroupsKey(myId), gid],
          ["SET", groupMetaKey(gid), meta],
        ];
    const results = await redisPipeline(cmds);
    if (!results || !Array.isArray(results) || results.some((r) => r && r.error)) {
      return res.status(500).json({ ok: false, error: "Ошибка создания чата" });
    }
    const memberCount = otherId ? 2 : 1;
    return res.status(200).json({ ok: true, group: { id: gid, title, memberCount } });
  }

  if (req.method === "POST" && postAction === "groupJoin") {
    const gid = (body.groupId || body.group_id || "").toString().trim();
    if (!isValidGroupId(gid)) return res.status(400).json({ ok: false, error: "Некорректная ссылка чата" });
    const ex = await redisPipeline([["EXISTS", groupMetaKey(gid)]]);
    if (!ex || ex[0]?.result !== 1) return res.status(404).json({ ok: false, error: "Чат не найден" });
    const mem = await redisPipeline([["SISMEMBER", groupMembersKey(gid), myId]]);
    if (mem && mem[0]?.result === 1) {
      let title = "Группа";
      try {
        const gr = await redisPipeline([["GET", groupMetaKey(gid)]]);
        const metaRaw = gr && gr[0] && gr[0].result;
        if (metaRaw && typeof metaRaw === "string") {
          const o = JSON.parse(metaRaw);
          if (o && o.title) title = String(o.title).slice(0, 60);
        }
      } catch (e) {}
      return res.status(200).json({ ok: true, alreadyMember: true, group: { id: gid, title } });
    }
    const card = await redisPipeline([["SCARD", groupMembersKey(gid)]]);
    const n = card && card[0]?.result != null ? parseInt(String(card[0].result), 10) : 0;
    if (n >= MAX_GROUP_MEMBERS) return res.status(400).json({ ok: false, error: "В чате максимум участников" });
    const results = await redisPipeline([
      ["SADD", groupMembersKey(gid), myId],
      ["SADD", userGroupsKey(myId), gid],
    ]);
    if (!results || results.some((r) => r && r.error)) {
      return res.status(500).json({ ok: false, error: "Не удалось вступить" });
    }
    let title = "Группа";
    try {
      const gr = await redisPipeline([["GET", groupMetaKey(gid)]]);
    ...
    return res.status(200).json({ ok: true, group: { id: gid, title } });
  }

  if (req.method === "POST" && postAction === "groupAddMember") {
    const gid = (body.groupId || body.group_id || "").toString().trim();
    const memberRaw = (body.memberId || body.member_id || body.userId || "").toString().trim();
    if (!isValidGroupId(gid)) return res.status(400).json({ ok: false, error: "Некорректный чат" });
    if (!memberRaw) return res.status(400).json({ ok: false, error: "Укажите игрока" });
    const okMem = await redisPipeline([["SISMEMBER", groupMembersKey(gid), myId]]);
    if (!okMem || okMem[0]?.result !== 1) return res.status(403).json({ ok: false, error: "Нет доступа к чату" });
    let otherId = memberRaw.startsWith("tg_") ? memberRaw : "tg_" + memberRaw.replace(/^tg_/i, "");
    if (!otherId.startsWith("tg_")) return res.status(400).json({ ok: false, error: "Некорректный ID" });
    if (otherId === myId) return res.status(400).json({ ok: false, error: "Вы уже в чате" });
    const already = await redisPipeline([["SISMEMBER", groupMembersKey(gid), otherId]]);
    if (already && already[0]?.result === 1) return res.status(400).json({ ok: false, error: "Игрок уже в чате" });
    const card = await redisPipeline([["SCARD", groupMembersKey(gid)]]);
    const n = card && card[0]?.result != null ? parseInt(String(card[0].result), 10) : 0;
    if (n >= MAX_GROUP_MEMBERS) return res.status(400).json({ ok: false, error: "В чате максимум участников" });
    const results = await redisPipeline([
      ["SADD", groupMembersKey(gid), otherId],
      ["SADD", userGroupsKey(otherId), gid],
    ]);
    if (!results || results.some((r) => r && r.error)) {
      return res.status(500).json({ ok: false, error: "Ошибка добавления" });
    }
    return res.status(200).json({ ok: true, memberCount: n + 1 });
  }
  ...
  if ((!text || text.length > 500) && !image && !voice && !document) {
    return res.status(400).json({ ok: false, error: "Текст от 1 до 500 символов, картинка, голосовое или документ PDF" });
  }
  if (text && text.length > 500) {
    return res.status(400).json({ ok: false, error: "Текст до 500 символов" });
  }

  const postGroupId = (body.groupId || body.group_id || "").toString().trim();
  if (postGroupId && isValidGroupId(postGroupId)) {
    const memChk = await redisPipeline([["SISMEMBER", groupMembersKey(postGroupId), myId]]);
    if (!memChk || memChk[0]?.result !== 1) {
      return res.status(403).json({ ok: false, error: "Нет доступа к чату" });
    }
    const key = groupMsgKey(postGroupId);
