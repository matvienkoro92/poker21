...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST" && postAction === "groupCreate") {
    ...
    if (!memberRaw) return res.status(400).json({ ok: false, error: "Добавьте одного игрока" });
    ...
    if (otherId === myId) return res.status(400).json({ ok: false, error: "Нельзя создать чат только с собой" });
    ...
    return res.status(200).json({ ok: true, group: { id: gid, title, memberCount: 2 } });
  }
