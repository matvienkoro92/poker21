...
module.exports = async function handler(req, res) {
  ...
  // GET ?userId=tg_xxx — публичная карточка пользователя (имя, P21, personal, isFriend)
  if (searchUserId && searchUserId.startsWith("tg_")) {
    ...
    return res.status(200).json({ ok: true, userId: targetId, userName, p21Id: p21Id || null, personalInfo: personalInfo || null, isFriend: !!isFriend });
  }
  ...
  const telegramUsername = getUsernameFromInitData(initData);
  if (telegramUsername) {
