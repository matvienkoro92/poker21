...
function analyze(inputPeriods) {
  ...
  const all = [...players.values()].filter(p => p.weeks.size).map(p => {
    ...
    return { baselineRake, baselineReady, baselineWeeks: baseline.length,
      ...
      windowActiveWeeks: windowIndexes.filter(i => p.weeks.has(i)).length,
  ...
  const stablekey = key.filter(p => p.active && p.windowActiveWeeks >= 2);
  const newkey = key.filter(p => p.active && p.windowActiveWeeks < 2);
  const support = [];
  const currentKeyIds = new Set(key.map(p => p.id));
  let extendedRake = keyRake;
  for (const p of ranked) {
    if (currentKeyIds.has(p.id)) continue;
    if (extendedRake >= fourWeekTotal * .9) break;
    extendedRake += p.totalRake;
    if (p.active && p.windowActiveWeeks >= 3) support.push(p);
  }
  ...
  return { comparable, currentTotal, previousTotal, fourWeekTotal, keyRakeShare: fourWeekTotal > 0 ? keyRake / fourWeekTotal * 100 : 0,
    keyWindowWeeks: 4, coverageWeeks, baselineCoverageWeeks: baselineIndexes.length, breakdown, key, keyall: key, stablekey, newkey, support, growth, decline, attention, periods, players: all, active: all.filter(p => p.active), inactive,
