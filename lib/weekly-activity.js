...
function analyze(periods) {
  ...
  const all = [...players.values()].filter(p => p.weeks.size).map(p => {
    ...
    return { id: p.id, nick: p.nick, active: last === 0, activeWeeks: p.weeks.size,
      totalRake: [...p.rake.entries()].filter(([index]) => index < keyWindow).reduce((sum, [, rake]) => sum + Math.max(0, rake), 0),
  ...
  const attention = all.filter(p => (p.key && !p.active && Array.isArray(periods[0]?.rows)) ||
    (decline.includes(p) && (p.key || -p.delta >= previousTotal * 0.01)));
  for (const p of attention) {
    p.priorityAmount = p.active ? -p.delta : p.lastRake;
    p.attentionReason = p.active ? 'Заметное снижение рейка' : 'Ключевой игрок без игры в отчётную неделю';
  }
  attention.sort((a,b) => b.priorityAmount-a.priorityAmount || Number(b.key)-Number(a.key) || a.id.localeCompare(b.id));
  ...
  return { comparable, currentTotal, previousTotal, fourWeekTotal, keyRakeShare: fourWeekTotal > 0 ? keyRake / fourWeekTotal * 100 : 0,
    keyWindowWeeks: Math.min(keyWindow, periods.length), key, growth, decline, attention, periods, players: all, active: all.filter(p => p.active), inactive,
