...
test("/джекпот выводит общий сбор суперюниона, лиги, сверку, типы и чистый остаток", async (t) => {
  ...
  assert.match(sentMessage.text, /<b>Общий джекпот по всем лигам: 383 370,28<\/b>/);
  ...
  assert.match(sentMessage.text, /Проверка: сумма по лигам 383 370,28 = общий джекпот 383 370,28/);
  assert.match(sentMessage.text, /Обычный джекпот в лиге Антирег — 171 646,96/);
  assert.match(sentMessage.text, /У остальных лиг — 137 067,32/);
  ...
  assert.match(sentMessage.text, /<b>Сборы минус выплаты: 300 275,38<\/b>/);
  ...
  assert.match(sentMessage.text, /<b>Полный итог: 294 051,38<\/b>$/);
});
