/**
 * Поиск пользователя по ID (формат ID + 6 цифр).
 * GET ?id=ID123456 с initData — вернуть userId для личного чата.
 */
...
module.exports = async function handler(req, res) {
  ...
  if (!searchId || !/^ID\d{6}$/.test(searchId)) {
    return res.status(400).json({ ok: false, error: "Введите ID в формате ID123456" });
  }
